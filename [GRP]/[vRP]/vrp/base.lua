...
print("^5[Galaxy]: ^7" .. 'Checking for vRP Updates..')
...
if err == 200 then
    ...
    if version ~= r_version then
        print("^5[Galaxy]: ^7" .. 'A Dunko Update is available from: https://github.com/DunkoUK/dunko_vrp')
    else 
        print("^5[Galaxy]: ^7" .. 'You are running the most up to date Dunko Version. Thanks for using Dunko_vRP and thanks to our contributors for updating the project. Support Found At: https://discord.gg/b8wQn2XqDt')
    end
else
    print("[Galaxy] unable to check the remote version")
end
end, "GET", "")
...
function vRP.getPlayerEndpoint(player)
    return GetPlayerEP(player) or vRP.kick(player,'No')
end
            ...
            if vRP.rusers[user_id] == nil then -- not present on the server, init
                ...
                vRP.getUData(user_id, "vRP:datatable", function(sdata)
                    ...
                    vRP.getLastLogin(user_id, function(last_login)
                        ...
                        print("[Galaxy] "..name.." ("..vRP.getPlayerEndpoint(source)..") joined (user_id = "..user_id..")")
                        TriggerEvent("vRP:playerJoin", user_id, source, name, tmpdata.last_login)
                        TriggerClientEvent("VRP:CheckIdRegister", source)
                    end)
                end)
            else -- already connected
                print("[Galaxy] "..name.." ("..vRP.getPlayerEndpoint(source)..") re-joined (user_id = "..user_id..")")
...
function vRP.banConsole(permid,time,reason)
    ...
    if getBannedPlayerSrc then 
        if tonumber(time) then 
            ...
            vRP.kick(getBannedPlayerSrc,"[Galaxy] You have been banned from Galaxy. \n\nYour ban will expire on: \n" .. os.date("%c", banTime) .. "\n\nReason: \n" .. reason .. "\n\nYou have been banned by: " .. "[Galaxy Anticheat]\n\n [Your ID: " .. permid .. "]") 
            print("~g~Success banned! User PermID:" .. permid)
        else 
            print("~g~Success banned! User PermID:" .. permid)
            vRP.setBanned(permid,true,"perm",reason,  'Console' .. " | ID Of Admin: " .. adminPermID .. "\n\n [Your ID: " .. permid .. "]")
            vRP.kick(getBannedPlayerSrc,"[Galaxy] You have been permanently banned from Galaxy. \n\nReason: \n" .. reason .. "\n\nYou have been banned by: " .. "[Galaxy Anticheat]\n\n [Your ID: " .. permid .. "]") 
        end
    else 
        if tonumber(time) then 
            local banTime = os.time()
...
AddEventHandler("playerConnecting",function(name,setMessage, deferrals)
    ...
    if ids ~= nil and #ids > 0 then
        deferrals.update("[Galaxy] Checking identifiers...")
        vRP.getUserIdByIdentifiers(ids, function(user_id)
            vRP.IdentifierBanCheck(source, user_id, function(status, id)
                if status then
                    print("[Galaxy] User rejected for attempting to evade ID: " .. user_id .. " | (Ignore joined message, they were rejected)") 
                    deferrals.done("[Galaxy]: You are banned from this server, please do not try to evade your ban. If you believe this was an error quote your ID which is: " .. id)
                    return 
                end
            end)
            ...
            if user_id ~= nil then -- check user validity 
                deferrals.update("[Galaxy] Fetching Tokens...")
                vRP.StoreTokens(source, user_id) 
                deferrals.update("[Galaxy] Checking banned...")
                vRP.isBanned(user_id, function(banned)
                    if not banned then
                        deferrals.update("[Galaxy] Checking whitelisted...")
                            ...
                            if not config.whitelist or whitelisted then
                                ...
                                if vRP.rusers[user_id] == nil then -- not present on the server, init
                                    if vRP.CheckTokens(source, user_id) then 
                                        deferrals.done("[Galaxy]: You are banned from this server, please do not try to evade your ban.")
                                    end
                                    vRP.users[ids[1]] = user_id
                                    vRP.rusers[user_id] = ids[1]
                                    vRP.user_tables[user_id] = {}
                                    ...
                                    deferrals.update("[Galaxy] Loading datatable...")
                                    vRP.getUData(user_id, "vRP:datatable", function(sdata)
                                        local data = json.decode(sdata)
                                        if type(data) == "table" then vRP.user_tables[user_id] = data end
                                        ...
                                        deferrals.update("[Galaxy] Getting last login...")
                                        vRP.getLastLogin(user_id, function(last_login)
                                            tmpdata.last_login = last_login or ""
                                            tmpdata.spawns = 0
                                ...
                                print("[Galaxy] "..name.." ("..vRP.getPlayerEndpoint(source)..") rejected: not whitelisted (user_id = "..user_id..")")
                                deferrals.done("[Galaxy] Not whitelisted (user_id = "..user_id..").")
                            end
                        end)
                    else
                        deferrals.update("[Galaxy] Fetching Tokens...")
                        ...
                        vRP.fetchBanReasonTime(user_id,function(bantime, banreason, banadmin)
                            if tonumber(bantime) then 
                                    ...
                                    if vRP.rusers[user_id] == nil then -- not present on the server, init
                                        ...
                                        vRP.getUData(user_id, "vRP:datatable", function(sdata)
                                            ...
                                            vRP.getLastLogin(user_id, function(last_login)
                                                ...
                                                -- trigger join
                                                print("[Galaxy] "..name.." ("..vRP.getPlayerEndpoint(source)..") joined after his ban expired. (user_id = "..user_id..")")
                                                TriggerEvent("vRP:playerJoin", user_id, source, name, tmpdata.last_login)
                                                deferrals.done()
                                            end)
                                        end)
                                    else -- already connected
                                        print("[Galaxy] "..name.." ("..vRP.getPlayerEndpoint(source)..") re-joined after his ban expired.  (user_id = "..user_id..")")
                                        TriggerEvent("vRP:playerRejoin", user_id, source, name)
                                        deferrals.done()
                                ...
                                print("[Galaxy] "..name.." ("..vRP.getPlayerEndpoint(source)..") rejected: banned (user_id = "..user_id..")")
                                deferrals.done("[Galaxy] You have been banned from this server.\nYour ban will expire on the: " .. os.date("%c", bantime) .. "\nReason: " .. banreason .. "\n\nBanning Admin: " .. banadmin)
                            else 
                                print("[Galaxy] "..name.." ("..vRP.getPlayerEndpoint(source)..") rejected: banned (user_id = "..user_id..")")
                ...
                print("[Galaxy] "..name.." ("..vRP.getPlayerEndpoint(source)..") rejected: identification error")
                deferrals.done("[Galaxy] Identification error.")
            end
        end)
    else
        print("[Galaxy] "..name.." ("..vRP.getPlayerEndpoint(source)..") rejected: missing identifiers")
        deferrals.done("[Galaxy] Missing identifiers.")
    end
    Debug.pend()
end)
...
AddEventHandler("playerDropped",function(reason)
    ...
    if user_id ~= nil then
        ...
        print("[Galaxy] "..vRP.getPlayerEndpoint(source).." disconnected (user_id = "..user_id..")")
        vRP.users[vRP.rusers[user_id]] = nil
        vRP.rusers[user_id] = nil
        vRP.user_tables[user_id] = nil
        vRP.user_tmp_tables[user_id] = nil
        vRP.user_sources[user_id] = nil
        print('[Galaxy] Player Leaving Save:  Saved data for: ' .. GetPlayerName(source))
    else 
        print('[Galaxy] SEVERE ERROR: Failed to save data for: ' .. GetPlayerName(source) .. ' Rollback expected!')
    end
