...
AddEventHandler('Galaxy:FetchTrunkInventory', function(spawnCode)
    local source = source
...
RegisterNetEvent('Galaxy:FetchTrunkInventory2')
AddEventHandler('Galaxy:FetchTrunkInventory2', function(spawnCode)
    local source = source
    local user_id = vRP.getUserId({source})
    if InventoryCoolDown[source] then vRPclient.notify(source, {'~r~The server is still processing your request.'}) return end
    local carformat = "chest:u1veh_" .. spawnCode .. '|' .. user_id
    vRP.getSData({carformat, function(cdata)
        ...
        local maxVehKg = Inventory.vehicle_chest_weights[spawnCode] or 1500
        TriggerClientEvent('Galaxy:SendSecondaryInventoryData', source, FormattedInventoryData, vRP.computeItemsWeight({cdata}), 1500)
    end})
end)
...
AddEventHandler('Galaxy:MoveItemAll', function(inventoryType, itemId, inventoryInfo, Lootbag)
    local source = source
    local UserId = vRP.getUserId({source}) 
    local data = vRP.getUserDataTable({UserId})
    if not itemId then  vRPclient.notify(source, {'~r~You need to select an item, first!'}) return end
