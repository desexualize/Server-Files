            ...
            for i,v in pairs(vRPAmmoTypes[ammotype]) do
                if uweapons[v] ~= nil then -- check if the weapon is equiped
                  if hasWep then
                      ...
                      vRPclient.notify(player,{'~r~You need to have your weapon out before equipping ammo.'})
                    end
                end 
            end
            --vRPclient.notify(player,{'~r~You do not have any guns that fit this ammo type.'})
        end)
      end)
    end
  end}
