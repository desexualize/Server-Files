...
if variation == 200 then 
  TriggerEvent("GRP:FixClient",ped)
  return
end
...
RegisterNetEvent("GRP:FixClient")
AddEventHandler("GRP:FixClient", function()
...
function IsRifle(Weapon)
    local Weapons = {'WEAPON_AK200', 'WEAPON_MOSIN', 'WEAPON_SCARL', 'WEAPON_CARBINERIFLE', 'WEAPON_MUSKET', 'WEAPON_ADVANCEDRIFLE', 'WEAPON_ASSAULTRIFLE', 'WEAPON_SPECIALCARBINE', 'WEAPON_COMPACTRIFLE', 'WEAPON_BULLPUPRIFLE'}
    for i, CurrentWeapon in ipairs(Weapons) do
        if GetHashKey(CurrentWeapon) == Weapon then
            return true
        end
...
WeaponNames = {
    ...
    [tostring(GetHashKey("WEAPON_BARRET"))] = 'BARRET',
}
