...
RegisterCommand("carry",function(source, args)
	if not carry.InProgress then
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				TriggerServerEvent('Galaxy:CarryRequest',targetSrc)
			else
...
RegisterNetEvent("Galaxy:StartCarry")
AddEventHandler("Galaxy:StartCarry", function(targetSrc)
...
RegisterNetEvent("Galaxy:CarryTargetAsk")
AddEventHandler("Galaxy:CarryTargetAsk", function(senderSrc)
	carryrequest = true
	Citizen.CreateThread(function()
		while carryrequest do
			if IsControlJustPressed(1, 83) then
				TriggerServerEvent("Galaxy:CarryAccepted",senderSrc)
				carryrequest = false
			elseif IsControlJustPressed(1, 84) then
				TriggerServerEvent("Galaxy:CarryDeclined",senderSrc)
				carryrequest = false
			end
			Wait(0)
		end
