RMenu.Add('KnifeStoreMenu', 'main', RageUI.CreateMenu("", "~r~Galaxy Kinife Store", 1300, 50, "knife", "knife"))
...
RageUI.CreateWhile(1.0, RMenu:Get("KnifeStoreMenu", "confirm"), nil, function()
    RageUI.IsVisible(RMenu:Get("KnifeStoreMenu", "confirm"), true, false, true, function()  
        ...
        RageUI.Button("Confirm" , nil, {RightLabel = ""}, true, function(Hovered, Active, Selected)
            if Selected then
                TriggerServerEvent('Galaxy:BuyKnife', cPrice, cHash)
            end
...
RegisterNetEvent("Galaxy:GiveKnife")
AddEventHandler("Galaxy:GiveKnife", function(hash)
     -- AntiCheat Allow Weapon Trigger
    GiveWeaponToPed(PlayerPedId(), hash, 150, false, false, 0)
end)

RegisterNetEvent("Galaxy:SetArmour")
AddEventHandler("Galaxy:SetArmour", function(level) 
