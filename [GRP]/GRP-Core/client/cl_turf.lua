...
RegisterNetEvent('GRPTurf:TakenTurf')
AddEventHandler('GRPTurf:TakenTurf', function(isnTurf)
...
RegisterNetEvent('GRPTurf:OutOfZone')
AddEventHandler('GRPTurf:OutOfZone', function(isnTurf)
	inTurf = false

	-- TriggerEvent('chatMessage', '[Galaxy Turf System]:', {255, 34, 0}, "The turf cap was cancelled, you will receive nothing.")
...
RegisterNetEvent('GRPTurf:PlayerDied')
AddEventHandler('GRPTurf:PlayerDied', function(isnTurf)
	inTurf = false
	-- TriggerEvent('chatMessage', '[Galaxy Turf System]:', {255, 34, 0}, "The turf cap was cancelled, you died!")
...
RegisterNetEvent('GRPTurf:TurfComplete')
AddEventHandler('GRPTurf:TurfComplete', function(reward, name)
	inTurf = false
	-- TriggerEvent('chatMessage', '[Galaxy Turf System]:', {255, 34, 0}, "Turf capture sucessful, you received:^2 £" .. reward)
...
Citizen.CreateThread(function()
	while true do
				...
				if not inTurf then
					...
					if (Vdist(pos.x, pos.y, pos.z, pos3.x, pos3.y, pos3.z) < 1.4) then
						...
						if (IsControlJustReleased(1, 51)) then
							TriggerServerEvent('GRPTurf:rob', k)
		...
		if inTurf then		
			...
			TriggerServerEvent('GRPTurf:PlayerDied', turf)

			elseif (Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) > turfs[turf].radius) then
				TriggerServerEvent('GRPTurf:TooFar', turf)
			end
		end
...
AddEventHandler('LSDZone', function(isnTurf)
	...
	while true do
		if isInTurfTing then
			DrawMarker(1, 2483.0139160156,-417.52560424805,93.735275268555-10, 0, 0, 0, 0, 0, 0, 80.001, 80.0001, 150.5001, 0, 255, 68, 30, 0, 0, 0, 0)
		end
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		--DrawMarker(1, 3572.0119628906,3649.6362304688,33.889598846436-1, 0, 0, 0, 0, 0, 0, 60.001, 60.0001, 50.5001, 255, 0, 0, 50, 0, 0, 0, 0)
		DrawMarker(24, 3572.0119628906,3649.6362304688,33.889598846436+1 - 0.98, 0, 0, 0, 0, 0, 0, 0.8, 0.8, 0.8, 255, 34, 0, 200, 0, 0, 0, 0, 0, 0, 0)
		...
		DrawMarker(24, 2477.9226074219,-420.8518371582,93.735176086426+1 - 0.98, 0.8, 0.8, 0, 0, 0, 0, 1.0, 1.0, 0.8, 255, 34, 0, 200, 0, 0, 0, 0, 0, 0, 0) 
