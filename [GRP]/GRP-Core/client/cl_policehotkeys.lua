...
RegisterNetEvent('Galaxy:policeHotkeysTrue')
AddEventHandler('Galaxy:policeHotkeysTrue', function() 
    handcuffed = true
end)

RegisterNetEvent('Galaxy:policeHotkeysFalse')
AddEventHandler('Galaxy:policeHotkeysFalse', function() 
    handcuffed = false
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlPressed(0, 19, true) and IsControlPressed(0, 30, true) then
            TriggerServerEvent('Galaxy:policeDrag')
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlPressed(0, 344, true) then
            TriggerServerEvent('Galaxy:policeCuff')
            Wait(1000)
        end
    end
end)

RegisterNetEvent("Galaxy:cuffAnimPolice")
AddEventHandler("Galaxy:cuffAnimPolice", function(nplayer, player)
    local player = player
    local nplayer = nplayer
    TriggerServerEvent("Galaxy:PlayWithinDistance", 0.2 , "cuffing", 1, GetEntityCoords(GetPlayerPed(-1))) 
...
RegisterNetEvent("Galaxy:cuffAnimPerp")
AddEventHandler("Galaxy:cuffAnimPerp", function()
...
RegisterNetEvent("Galaxy:drag")
AddEventHandler('Galaxy:drag', function(pl)
    other = pl
    drag = not drag
end)

RegisterNetEvent("Galaxy:undrag")
AddEventHandler('Galaxy:undrag', function(pl)
    drag = false
end)
