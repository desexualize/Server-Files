...
vRPclient = Tunnel.getInterface("vRP", "GRPTurf")

local isPlayerInTurf = {}

local completedTurf = false

RegisterServerEvent('GRPTurf:TooFar')
AddEventHandler('GRPTurf:TooFar', function(isnTurf)
	...
	if turfs[isnTurf] then
		...
		if(isPlayerInTurf[source]) then
			TriggerClientEvent('GRPTurf:OutOfZone', source)
...
RegisterServerEvent('GRPTurf:PlayerDied')
AddEventHandler('GRPTurf:PlayerDied', function(isnTurf)
	...
	if turfs[isnTurf] then
		local turf = turfs[isnTurf]
		if(isPlayerInTurf[source])then
			TriggerClientEvent('GRPTurf:PlayerDied', source)
			isPlayerInTurf[source] = nil
			TriggerClientEvent('chat:addMessage', -1, {
				template = '<div style="padding: 0.5vw; margin: 0.4vw; background-color: rgba(255, 0, 0, 0.6); border-radius: 4px;"><i class="fas fa-globe"></i> Turfs^7: ' .. "Turf capture sucessful at ^2" .."Turf capture failed at ^3" .. turf.nameofturf .. '</div>',
				args = { playerName, msg }
...
AddEventHandler("changeCom", function(com)
	if completedTurf == true then
		if com <= 35 then
			if com >= 0 then
				TriggerClientEvent('chatMessage', -1, '[Turf Infomation]:', {255, 34, 0}, "^3" .. GetPlayerName(source) .. " ^0has changed Commision to ^3" .. com .. "% ^0at Large Arms", "alert")
				TriggerClientEvent('chat:addMessage', -1, {
...
RegisterServerEvent('GRPTurf:rob')
AddEventHandler('GRPTurf:rob', function(isnTurf)
	local user_id = vRP.getUserId({source})
	local player = vRP.getUserSource({user_id})
	  ...
	  if turfs[isnTurf] then
		...
		if (os.time() - turf.lastisnTurfed) < 150 and turf.lastisnTurfed ~= 0 then
			...
			vRPclient.notify(player,{"~g~Defend the area for ~w~120 ~g~Seconds and the Turf is yours!"})
		  	TriggerClientEvent('GRPTurf:TakenTurf', player, isnTurf)
			  TriggerClientEvent("doneIt", -1, false)
			  	...
			  	if(isPlayerInTurf[savedSource]) then
				  	...
				  	if(user_id) then
						...
						if turf.nameofturf == "Weed" then
					 	...
					 	TriggerClientEvent('GRPTurf:TurfComplete', savedSource, turf.reward, turf.nameofturf)
						 TriggerClientEvent("turfTrue", -1, false)
						 completedTurf = true
				  	end
