...
RegisterNetEvent('GRP:policeDrag')
AddEventHandler('GRP:policeDrag', function()
    ...
    if user_id ~= nil and vRP.hasPermission({user_id, "police.menu"}) then
        ...
        vRPclient.getNearestPlayer(source,{2},function(nplayer)
            ...
            if nuser_id ~= nil then
                ...
                if handcuffed then
                    TriggerClientEvent("GRP:drag2", nplayer, source)
                else
...
RegisterNetEvent("GRP:policeCuff")
AddEventHandler("GRP:policeCuff", function()
    local player = source
    local user_id = vRP.getUserId({source})
    handcuffed = nil
    if user_id ~= nil and vRP.hasPermission({user_id, "police.menu"}) then
            ...
            vRPclient.isHandcuffed(nplayer,{},function(handcuffed)
                ...
                if nuser_id ~= nil then
                    if handcuffed == false then
                        ...
                        TriggerClientEvent("GRP:cuffAnimPolice", player, nplayer, player)
                        TriggerClientEvent("GRP:cuffAnimPerp", nplayer, nplayer, player)
...
RegisterServerEvent('GRP:toggleTrafficMenu')
AddEventHandler('GRP:toggleTrafficMenu', function()
    local source = source
    local user_id = vRP.getUserId({source})
    if user_id ~= nil and vRP.hasPermission({user_id, "police.menu"}) then
        print("vRP Identification Accepted") 
        TriggerClientEvent("GRP:openTrafficMenu", source)
    end
end)

RegisterServerEvent('GRP:zoneActivated')
AddEventHandler('GRP:zoneActivated', function(message, speed, radius, x, y, z)
    TriggerClientEvent('chatMessage', -1, message)
    TriggerClientEvent('GRP:createZone', -1, speed, radius, x, y, z)
end)

RegisterServerEvent('GRP:disableZone')
AddEventHandler('GRP:disableZone', function(blip)
    TriggerClientEvent('GRP:removeBlip', -1)
end)
...
RegisterServerEvent('GRP:Jail')
AddEventHandler('GRP:Jail', function() 
    player = source
                ...
                if target ~= nil then
                  ...
                  vRPclient.isHandcuffed(target,{}, function(handcuffed)  
                    if handcuffed then 
                      ...
                      vRPclient.teleport(target,{1715.16, 2586.81, 53.19}) -- teleport to inside jail
...
RegisterServerEvent('GRP:Fine')
AddEventHandler('GRP:Fine', function() 
    player = source
    local user_id = vRP.getUserId({player})
    if user_id ~= nil and vRP.hasPermission({user_id, "police.menu"}) then
    vRPclient.getNearestPlayers(player,{15},function(nplayers) 
...
RegisterNetEvent("GRP:Unjail")
AddEventHandler("GRP:Unjail", function()
    player = source
    local user_id = vRP.getUserId({player})
    if user_id ~= nil and vRP.hasPermission({user_id, "police.menu"}) then
...
RegisterNetEvent("GRP:putinveh")
AddEventHandler("GRP:putinveh", function()
    player = source
    local user_id = vRP.getUserId({player})
    if user_id ~= nil and vRP.hasPermission({user_id, "police.menu"}) then
...
RegisterNetEvent("GRP:takeoutveh")
AddEventHandler("GRP:takeoutveh", function()
    player = source
    local user_id = vRP.getUserId({player})
    if user_id ~= nil and vRP.hasPermission({user_id, "police.menu"}) then
  vRPclient.getNearestPlayer(player,{10},function(nplayer)
...
RegisterServerEvent('GRP:Drag')
AddEventHandler('GRP:Drag', function() 
      -- get nearest player
      local user_id = vRP.getUserId({player})
      if user_id ~= nil then
        vRPclient.getNearestPlayer(player,{10},function(nplayer)
...
RegisterServerEvent('GRP:SeizeWeapons')
AddEventHandler('GRP:SeizeWeapons', function() 
  local player = source
  local user_id = vRP.getUserId(player)
  if user_id ~= nil then
...
--[[
RegisterServerEvent('GRP:SeizeItems')
AddEventHandler('GRP:SeizeItems', function()
  local player = source 
  local user_id = vRP.getUserId({player})
  if user_id ~= nil then
    vRPclient.getNearestPlayer(player, {5}, function(nplayer)
...
RegisterServerEvent('GRP:TrafficMenuLogs')
AddEventHandler('GRP:TrafficMenuLogs', function(objspawned) 
  local player = source
  local admin_id = vRP.getUserId({player})
  local admin_name = GetPlayerName(player)
  local logs = ""
  local communityname = "GRP RP Staff Logs"
  ...
  PerformHttpRequest(logs, function(err, text, headers) end, 'POST', json.encode({username = "GRP RP Staff Logs", embeds = command}), { ['Content-Type'] = 'application/json' })
end)