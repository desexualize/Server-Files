...
RegisterNetEvent('Galaxy:policeDrag')
AddEventHandler('Galaxy:policeDrag', function()
    ...
    if user_id ~= nil and vRP.hasPermission({user_id, "police.menu"}) then
        ...
        vRPclient.getNearestPlayer(source,{2},function(nplayer)
            ...
            if nuser_id ~= nil then
                ...
                if handcuffed then
                    TriggerClientEvent("Galaxy:drag", nplayer, source)
                else
...
RegisterNetEvent("Galaxy:policeCuff")
AddEventHandler("Galaxy:policeCuff", function()
    local player = source
    local user_id = vRP.getUserId({source})
    handcuffed = nil
    if user_id ~= nil and vRP.hasPermission({user_id, "police.menu"}) then
            ...
            vRPclient.isHandcuffed(nplayer,{},function(handcuffed)
                ...
                if nuser_id ~= nil then
                    if handcuffed == false then
                        ...
                        TriggerClientEvent("Galaxy:cuffAnimPolice", player, nplayer, player)
                        TriggerClientEvent("Galaxy:cuffAnimPerp", nplayer, nplayer, player)
