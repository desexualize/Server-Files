...
RMenu.Add('TruckJob', 'main', RageUI.CreateMenu("Truck Job", "Galaxy Truck Job", 1300, 50))

RageUI.CreateWhile(1.0, RMenu:Get('TruckJob', 'main'), nil, function()

    RageUI.IsVisible(RMenu:Get('TruckJob', 'main'), true, false, true, function()



        RageUI.Button("Illegal Truck Job" , nil, {RightLabel = ""}, true, function(Hovered, Active, Selected)
            if Selected then
                GUI.optionMisson(2)
                GUI.mission(2)
        ...
        RageUI.Button("Legal Truck Job" , nil, {RightLabel = ""}, true, function(Hovered, Active, Selected)
            if Selected then
                GUI.optionMisson(2)
                GUI.mission(2)
...
TruckingCompany[0] = {["x"] = 936.77331542969,["y"] = -3196.982421875, ["z"] = -3196.982421875,5.9007587432861}
local TruckingTrailer = {}
TruckingTrailer[0] = {["x"] = 930.83990478516, ["y"] = -3148.1416015625, ["z"] = 5.9007935523987} 

local Truck = {"HAULER", "PACKER", "PHANTOM"}
local Trailer = {"TANKER", "TRAILERS", "TRAILERS2", "TRAILERLOGS"}

local MissionData = {
    [0] = {-436.52386474609,1585.5860595703,357.62091064453, 5000}, --x,y,z,money
    [1] = {-436.52386474609,1585.5860595703,357.62091064453, 10000},
    [2] = {-436.52386474609,1585.5860595703,357.62091064453, 50000},
...
function tick()
    ...
    if( GetDistanceBetweenCoords( playerCoords, TruckingCompany[0]["x"], TruckingCompany[0]["y"], TruckingCompany[0]["z"] ) < 3.5) then
            if(GUI.showStartText == false) then
                alert('Press ~INPUT_VEH_HORN~ to open Truck Job Menu.')
            end
                --key controlling
                if(IsControlPressed(1, 51) and GUI.showMenu == false) then
                    --clear()
                    RageUI.Visible(RMenu:Get("TruckJob", "main"), true)
                end
            else
                GUI.showStartText = false
...
function MISSION.spawnTruck()
    ...
    MISSION.truck = CreateVehicle(MISSION.hashTruck, 930.04858398438,-3119.5952148438,5.9008030891418, 0.0, true, false)
...
function GUI.init()
    ...
    GUI.addButton(0, "Truck Job", GUI.optionMisson, 0.35, 0.25, 0.3, 0.05 )
   -- GUI.addButton(0, "Exit Menu", GUI.exit, 0.35, 0.50, 0.3, 0.05 )
    
    GUI.buttonCount = 0
    
    GUI.addButton(1, "Illegal Truck Job [ £50,000 ]", GUI.mission, 0.35, 0.25, 0.3, 0.05)
    GUI.addButton(1, "Legal Truck Job [ £20,000 ]", GUI.mission, 0.35, 0.30, 0.3, 0.05)
