...
if (process.env.TOKEN == "" || process.env.TOKEN == "TOKEN") {
    console.log(`[^1GalaxyRP Discord Bot^7]: Error! No Token Provided you forgot to edit the .env`);
    throw new Error('Whoops!')
}

client.on('ready', () => {
    console.log(`[^1GalaxyRP Discord Bot^7]: Logged in as ${client.user.tag}! Players: ${GetNumPlayerIndices()}`);
    console.log(`[^1GalaxyRP Discord Bot^7]: Your Prefix Is ${process.env.PREFIX}`)
    init()
});
...
if (settingsjson.settings.StatusEnabled) {
    setInterval(() => {
        if (!client.guilds.get(settingsjson.settings.GuildID)) return console.log(`[^1GalaxyRP Discord Bot^7]: Status is enabled but not configured correctly and will not work as intended.`)
        let channelid = client.guilds.get(settingsjson.settings.GuildID).channels.find(r => r.name === settingsjson.settings.StatusChannel);
        if (!channelid) return console.log(`[^1GalaxyRP Discord Bot^7]: Status channel is not available / cannot be found.`)
...
const init = async() => {
    fs.readdir(resourcePath + '/commands/', (err, files) => {
        ...
        console.log(`[^1GalaxyRP Discord Bot^7]: Loading a total of ${files.length} commands.`);
        files.forEach(f => {
...
client.getPerms = function(msg) {
    ...
    if (!lvl1 || !lvl2 || !lvl3) {
        console.log(`[^1GalaxyRP Discord Bot^7]: Your permissions are not setup correctly and the bot will not function as intended.\nStatus: Level 1 Perm is: ${lvl1}, Level 2 Perm is: ${lvl2}, Level 3 Perm is ${lvl3}`)
    }
