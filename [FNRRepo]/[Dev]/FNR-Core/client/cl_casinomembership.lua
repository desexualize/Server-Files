...
RMenu.Add("FNR:CasinoLicenseStore", "main", RageUI.CreateMenu("", "~b~Casino License Store", 1350, 50,"banners","licenses"))
RMenu.Add("FNR:CasinoLicenseStore", "sub", RageUI.CreateSubMenu(RMenu:Get("FNR:CasinoLicenseStore", "main"), "", "~b~License Store", 1350, 50))
RMenu.Add("FNR:CasinoLicenseStore", "remove", RageUI.CreateSubMenu(RMenu:Get("FNR:CasinoLicenseStore", "main"), "", "~b~Confirm Remove License", 1350, 50))
RMenu.Add("FNR:CasinoLicenseStore", "confirm", RageUI.CreateSubMenu(RMenu:Get("FNR:CasinoLicenseStore", "sub"), "", "~b~Confirm License Purchase", 1350, 50))

RageUI.CreateWhile(1.0, RMenu:Get("FNR:CasinoLicenseStore", "main"), nil, function()
    RageUI.IsVisible(RMenu:Get("FNR:CasinoLicenseStore", "main"), true, false, true, function()

        RageUI.Button("Casino Licenses", nil, {RightLabel = ">>>"}, true, function(Hovered, Active, Selected)
            if Selected then
                currentType = "casino"
            end
        end, RMenu:Get("FNR:CasinoLicenseStore", "sub", true))

    end, function() 
    end)

    RageUI.IsVisible(RMenu:Get("FNR:CasinoLicenseStore", "sub"), true, false, true, function()
            ...
            if v.type == currentType then
                RageUI.Button(v.name, nil, {RightLabel = "~g~£"..getMoneyStringFormatted(v.price)}, true, function(Hovered, Active, Selected)
                    if Selected then
                        ...
                        RMenu:Get("FNR:CasinoLicenseStore", "confirm"):SetSubtitle("~b~Are you sure you want to buy "..v.name.."?")
                    end
                end, RMenu:Get("FNR:CasinoLicenseStore", "confirm", true))
            end
        end
        ...
        end, RMenu:Get("FNR:CasinoLicenseStore", "remove", true))

    end, function() 
    end)

    RageUI.IsVisible(RMenu:Get("FNR:CasinoLicenseStore", "confirm"), true, false, true, function()
        ...
        end, RMenu:Get("FNR:CasinoLicenseStore", "main", true))
    ...
    RageUI.IsVisible(RMenu:Get("FNR:CasinoLicenseStore", "remove"), true, false, true, function()
        ...
        RageUI.Button("~g~Confirm Removal", nil, {RightLabel = "~g~£0"}, true, function(Hovered, Active, Selected)
            if Selected then
                TriggerServerEvent("FNR:RemoveLicense", currentLicense)
            end
        end, RMenu:Get("FNR:CasinoLicenseStore", "main", true))
    ...
    while true do
        ...
        if isInArea(cfg.casinolicenseShopLocation, 1.0) and inMenu == false then
            ...
            if IsControlJustPressed(0, 51) then 
                inMenu = true
                RageUI.Visible(RMenu:Get("FNR:CasinoLicenseStore", "main"), true)
            end
        end
