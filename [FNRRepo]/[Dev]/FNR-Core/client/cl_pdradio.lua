...
RegisterNetEvent('FNR:pdRadioAnim')
AddEventHandler('FNR:pdRadioAnim', function(status, b)
    if status then
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustReleased(0, 19) then
            TriggerServerEvent('FNR:pdRadioPerms', false, b)
        else
            if IsControlJustPressed(0, 19) then
                TriggerServerEvent('FNR:pdRadioPerms', true, b)
            end
        end
    end
end)
