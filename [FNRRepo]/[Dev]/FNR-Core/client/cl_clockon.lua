...
RMenu.Add('PDClockOn', 'main', RageUI.CreateMenu("", "~b~FNR PD Clockon", 1300, 50, "banners", "police"))
    ...
    RageUI.IsVisible(RMenu:Get('PDClockOn', 'main'), true, false, true, function()
        for i , p in pairs(clockon) do 
            RageUI.Button(p.group, nil, { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if Selected then
                    TriggerServerEvent('FNR:Clockon', p.group)
                    TriggerEvent('FNR:policeRemove')
                    TriggerServerEvent("FNR:PoliceCheckRadial")
                end
            end)
        end
...
RegisterNetEvent('FNR:policeRemove')
AddEventHandler('FNR:policeRemove', function()
    local source = source
