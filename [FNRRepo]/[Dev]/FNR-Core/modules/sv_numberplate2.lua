...
RegisterNetEvent('FNR:getCars')
AddEventHandler('FNR:getCars', function()
    local cars = {}
    local source = source
    ...
    exports['ghmattimysql']:execute("SELECT * FROM `vrp_user_vehicles` WHERE user_id = @user_id", {user_id = user_id}, function(result)
        if result ~= nil then 
            ...
            TriggerClientEvent('FNR:carsTable', source, cars)
        end
    end)
end)

RegisterNetEvent("FNR:ChangeNumberPlate")
AddEventHandler("FNR:ChangeNumberPlate", function(vehicle)
    local user_id = vRP.getUserId({source})
	...
	vRP.prompt({source,"Plate Name:","",function(source, plateName)
		...
		if vRP.tryFullPayment({user_id,50000}) then
			...
			TriggerClientEvent("FNR:RecieveNumberPlate", source, plateName)
			TriggerClientEvent("FNR:PlaySound", source, 1)
			TriggerEvent('FNR:getCars')
		else
