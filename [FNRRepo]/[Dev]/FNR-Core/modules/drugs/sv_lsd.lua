function FNRcfgdrugsServer.LSDGather()
...
function FNRcfgdrugsServer.CanProcessLSD()
    local user_id = vRP.getUserId({source})
...
function FNRcfgdrugsServer.ProcessLSD()
    local user_id = vRP.getUserId({source})
...
function FNRcfgdrugsServer.SellLSD(amount)
    local user_id = vRP.getUserId({source})
    if vRP.tryGetInventoryItem({user_id,'LSD', 1}) then
      ...
      if user_id ~= nil then
        ...
        local price = 130000 * tonumber(amount) *2 -- 65k
