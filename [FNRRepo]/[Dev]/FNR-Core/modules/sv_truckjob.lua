...
RegisterNetEvent('FNR:startTruckJob')
AddEventHandler('FNR:startTruckJob', function()
    local source = source
...
RegisterNetEvent('FNR:finishTruckJob')
AddEventHandler('FNR:finishTruckJob', function(jobname)
    local source = source
    local user_id = vRP.getUserId({source})
    if user_id ~= nil then
