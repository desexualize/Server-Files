                ...
                if b then
                    RageUI.Checkbox("Return to normal Universe", "~r~Leave Car Dev Mode", b, {}, function(Hovered, Active, Selected, Checked)
                        if Selected then
                            ...
                            b=false
                            TriggerServerEvent("FNR:setCarDev",b)
                        end
                    end)
                else
...
RegisterCommand('cardev', function()
    TriggerServerEvent('FNR:openCarDev')
end)

function tvRP.openCarDev()
...
function TakeVehScreenshot()
    ...
    Wait(3000)
    TriggerServerEvent('FNR:takeCarScreenshot')
    Wait(5000)
...
local A={
    ...
    {name="fPercentSubmFNRed",type="float"},
...
local function a1()
    ...
    TriggerServerEvent('FNR:getCarDevDebug',{a2})
end
