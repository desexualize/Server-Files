RMenu.Add('FNRPoliceMenu', 'main', RageUI.CreateMenu("","FNR Police",1250,100, "banners", "police"))
RMenu.Add('FNRPoliceMenu', 'objectmenu',  RageUI.CreateSubMenu(RMenu:Get("FNRPoliceMenu", "main")))
RMenu:Get('FNRPoliceMenu', 'main'):SetPosition(1300, 100)
RMenu:Get('FNRPoliceMenu', 'objectmenu'):SetPosition(1300, 100)
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('FNRPoliceMenu', 'objectmenu')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true}, function()
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('FNRPoliceMenu', 'speedzones')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true}, function()
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
                ...
                RageUI.Button("Delete Speedzone" , nil, {}, true, function(Hovered, Active, Selected) 
                    if Selected then 
                        TriggerServerEvent('FNR:RemoveZone')
...
RegisterCommand('pd', function()
  if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
    TriggerServerEvent('FNR:OpenPoliceMenu')
  end
end)
...
RegisterNetEvent("FNR:PoliceMenuOpened")
AddEventHandler("FNR:PoliceMenuOpened",function()
  RageUI.Visible(RMenu:Get('FNRPoliceMenu', 'main'), not RageUI.Visible(RMenu:Get('FNRPoliceMenu', 'main')))
end)
...
function createZone() 
  ...
  if speed == 0 then 
    ...
    return
      ...
      TriggerServerEvent('FNR:ActivateZone', message, speed, radius, x, y, z)
end

RegisterNetEvent('FNR:ZoneCreated')
AddEventHandler('FNR:ZoneCreated', function(speed, radius, x, y, z)
...
RegisterNetEvent('FNR:RemovedBlip')
AddEventHandler('FNR:RemovedBlip', function()
...
RegisterNetEvent("FNR:DragPlayer")
AddEventHandler('FNR:DragPlayer', function(pl)
    other = pl
    drag = not drag
end)
