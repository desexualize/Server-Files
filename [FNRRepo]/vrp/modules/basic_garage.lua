...
-- -- vehicle db
local lang = vRP.lang
local cfg = module("FNRCars", "cfg/cfg_garages")
...
Citizen.CreateThread(function()
    while true do
        ...
        print('[FNR] Vehicle Rent Check | Successful')
    end
end)
                                                    ...
                                                    vRP.request(target,GetPlayerName(player).." wants to sell: " ..name.. " Price: £"..amount, 10, function(target,ok)
                                                        if ok then
                                                            ...
                                                            if vRP.tryFullPayment(pID,amount) then
                                                                ...
                                                                webhook = "https://discord.com/api/webhooks/1027267071345696778/DfnmpULDAd-MBLxz5PuhwNsj53L8AYF1lnefDmZZXBZXnBsnX7Uc0Nsbf0e37MNwtgf5"
...
AddEventHandler('vRP:FetchFolders', function()
    local source = source
    local user_id = vRP.getUserId(source)
    exports["ghmattimysql"]:execute("SELECT * from `vrp_custom_garages` WHERE user_id = @user_id", {user_id = user_id}, function(Result)
        if #Result > 0 then
...
AddEventHandler('vRP:UpdateFolders', function(FolderUpdated)
    ...
    exports["ghmattimysql"]:execute("SELECT * from `vrp_custom_garages` WHERE user_id = @user_id", {user_id = user_id}, function(Result)
        if #Result > 0 then
            exports['ghmattimysql']:execute("UPDATE vrp_custom_garages SET folder = @folder WHERE user_id = @user_id", {folder = json.encode(FolderUpdated), user_id = user_id}, function() end)
        else
            exports['ghmattimysql']:execute("INSERT INTO vrp_custom_garages (`user_id`, `folder`) VALUES (@user_id, @folder);", {user_id = user_id, folder = json.encode(FolderUpdated)}, function() end)
        end
    end)
end)
...
RegisterNetEvent("FNR:PayVehicleTax")
AddEventHandler("FNR:PayVehicleTax", function()
    local user_id = vRP.getUserId(source)
    ...
    if user_id ~= nil then
        ...
        if bank == 0 then
            vRPclient.notify(source,{"~r~Its fine... Tax payers will pay your vehicle tax instead."})
        elseif vRP.tryBankPayment(user_id, payment) then
            vRPclient.notify(source,{"~g~Paid £"..getMoneyStringFormatted(roundnumber(payment, 0)).." vehicle tax."})
        end
    end
end)
...
function getMoneyStringFormatted(cashString)
	...
	int = int:reverse():gsub("(%d%d%d)", "%0.1,")
...
Citizen.CreateThread(function()
    ...
    exports['ghmattimysql']:execute([[
        CREATE TABLE IF NOT EXISTS `vrp_custom_garages` (
