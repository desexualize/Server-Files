local DISCORD_WEBHOOK = "https://discord.com/api/webhooks/1027268138489876642/5VGms88SwGMh0aAiVJHSTJ83VMvvE4Kc4tMK2yb5PIhUP04RiHRZOxz-QcRVkqWKgNh9"
local DISCORD_NAME = "FNR"
local DISCORD_IMAGE = ""
...
RegisterCommand("anon", function(source, args, raw)
    ...
    if anoncooldown[source] and not (os.time() > anoncooldown[source]) then
        TriggerClientEvent('chatMessage', source, "FNR",  { 255, 255, 255 }, "You are being rate limited.", "alert")
        return
    else
...
RegisterCommand("p", function(source,args, rawCommand)
    user_id2 = vRP.getUserId(source)   
    if not vRP.hasPermission(user_id2, "police.armoury") then
        local playerName = "Server "
        local message = "Access denied."
        TriggerClientEvent('chatMessage', source, "^7Alert: " , { 128, 128, 128 }, message, "alert")
    end
    if vRP.hasPermission(user_id2, "police.armoury") then
        local message = rawCommand:sub(2)
        local playerName =  GetPlayerName(source)
        local players = GetPlayers()
        for i,v in pairs(players) do 
...
function ooc(source,args,raw)
    ...
    elseif vRP.hasGroup(user_id, 'leaddev') then 
        TriggerClientEvent('chatMessage', -1, "^7^*OOC ^7^r | ^1Lead Developer ^0" .. GetPlayerName(source) .." : " , { 128, 128, 128 }, message, "ooc")
