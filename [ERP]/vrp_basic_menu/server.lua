  ...
  vRPclient.getNearestPlayer(player,{5},function(nplayer)
    ...
    if nuser_id ~= nil then
      ...
      vRPclient.getWeapons(nplayer,{},function(weapons)
        ...
        vRPclient.setDiv2(player,{"police_check",".div_police_check{ background-color: rgba(0,0,0,0.75); color: white; font-weight: bold; width: 500px; padding: 10px; margin: auto; margin-top: 150px; }",lang.police.menu.check.info({money,items,weapons_info})})
        -- request to hide div
...
RegisterServerEvent("ERP:SearchNearest")
AddEventHandler("ERP:SearchNearest", function()
  local player = source
  vRPclient.getNearestPlayer(player,{5},function(nplayer)
    local nuser_id = vRP.getUserId({nplayer})
    if nuser_id ~= nil then
	  ...
	  if value ~= nil then
	    ...
	    if custom ~= nil then
		  if tonumber(custom) > 0 then
			BMclient.loadFreeze(target,{false,false,false})
      TriggerClientEvent("ERP:UnFreezeRespawn", source)
			SetTimeout(1,function()
			  BMclient.loadFreeze(target,{false,false,false})
			end)
