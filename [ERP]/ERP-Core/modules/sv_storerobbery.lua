...
AddEventHandler('es_bank:toofar', function(robb)
	if(robbers[source])then
		...
		robbers[source] = nil
		TriggerClientEvent('chatMessage', -1, '[Store Robbery]:', {255, 0, 0}, "Robbery was cancelled at: ^2" .. banks[robb].nameofbank, "alert")
	end
end)
...
AddEventHandler('es_bank:rob', function(robb)
 ...
 -- change 2 to the minimum amount online necessary
	  if banks[robb] then
		  local bank = banks[robb]

		  if (os.time() - bank.lastrobbed) < 600 and bank.lastrobbed ~= 0 then
			  TriggerClientEvent('chatMessage', player, '[Store Robbery]:', {255, 0, 0}, "This store has already been robbed. You'll have to wait another: ^2" .. (1200 - (os.time() - bank.lastrobbed)) .. "^0 seconds.", "ooc")
			  TriggerClientEvent('chat:addMessage', -1, {
				template = '<div style="padding: 0.5vw; margin: 0.4vw; background-color: rgba(255, 0, 0, 0.6); border-radius: 4px;"><i class="fas fa-globe"></i> [Store Robbery]^7: ' .. "This store has already been robbed. You'll have to wait another: ^2" .. (1200 - (os.time() - bank.lastrobbed)) .. "^0 seconds." .. '</div>',
				args = { playerName, msg }
			  })
			  return
		  end
		  TriggerClientEvent('chatMessage', -1, '[Store Robbery]:', {255, 0, 0}, "Robbery in progress at ^2" .. bank.nameofbank, "alert")
		  TriggerClientEvent('chat:addMessage', -1, {
			template = '<div style="padding: 0.5vw; margin: 0.4vw; background-color: rgba(255, 0, 0, 0.6); border-radius: 4px;"><i class="fas fa-globe"></i> [Store Robbery]^7: ' .. "Robbery in progress at ^2" .. bank.nameofbank .. "" .. '</div>',
			args = { playerName, msg }
		  })
		  local source = source
		  local index = #pdCalls + 1
		  pdCalls[index] = {GetPlayerName(source), source, "~r~[Store Robbery]"}
		  TriggerClientEvent('CallManager:Table', -1, adminTickets, nhsCalls, pdCalls)
		  TriggerClientEvent('chatMessage', player, '[Store Robbery]:', {255, 0, 0}, "You have started a robbery! If you leave the shop it will be cancelled!", "ooc")
		  TriggerClientEvent('chat:addMessage', -1, {
			template = '<div style="padding: 0.5vw; margin: 0.4vw; background-color: rgba(255, 0, 0, 0.6); border-radius: 4px;"><i class="fas fa-globe"></i> [Store Robbery]^7: ' .. "You have started a robbery! If you leave the shop it will be cancelled!" .. '</div>',
			args = { playerName, msg }
		  })
			  ...
			  if(robbers[savedSource])then
				  if(user_id)then
					vRP.giveMoney(user_id,bank.reward)
					  TriggerClientEvent('chatMessage', -1, '[Store Robbery]:', {255, 0, 0}, "Robbery is over at: ^2" .. bank.nameofbank .. "^0!", "alert")
					  TriggerClientEvent('chat:addMessage', -1, {
						template = '<div style="padding: 0.5vw; margin: 0.4vw; background-color: rgba(255, 0, 0, 0.6); border-radius: 4px;"><i class="fas fa-globe"></i> [Store Robbery]^7: ' .. "Robbery is over at: ^2" .. bank.nameofbank .. "^0!" .. '</div>',
						args = { playerName, msg }
					  })
