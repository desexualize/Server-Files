...
RegisterServerEvent("SmallArms3:BuyWeapon")
AddEventHandler('SmallArms3:BuyWeapon', function(price, hash, bool)
    local source = source
    userid = vRP.getUserId({source})
    local coords = vec3(-1500.0544433594,-216.84588623047,47.889381408691)
    ...
    if #(playerCoords - coords) <= 5.0 then 
            if vRP.getInventoryWeight({userid}) <= 25 then
                if vRP.tryPayment({userid, price}) then
                    ...
                    TriggerClientEvent('small3:givegun', source, hash)
...
RegisterServerEvent("SmallArms3:BuyArmour")
AddEventHandler('SmallArms3:BuyArmour', function(bool)
    local source = source
    userid = vRP.getUserId({source})
    local ped = GetPlayerPed(source)
    local playerCoords = GetEntityCoords(ped)
...
RegisterServerEvent("SmallArms3:BuyArmourPlate")
AddEventHandler('SmallArms3:BuyArmourPlate', function()
    local source = source
    userid = vRP.getUserId({source})
    local ped = GetPlayerPed(source)
    local playerCoords = GetEntityCoords(ped)
