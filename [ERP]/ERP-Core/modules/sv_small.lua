...
AddEventHandler('SmallArms:BuyWeapon', function(price, hash, bool)
    ...
    local coords = vec3(1242.8059082031,-426.96542358398,68.91535949707)
...
RegisterServerEvent("SmallArms:BuyArmourPlate")
AddEventHandler('SmallArms:BuyArmourPlate', function()
    local source = source
    userid = vRP.getUserId({source})
    local ped = GetPlayerPed(source)
    local playerCoords = GetEntityCoords(ped)
