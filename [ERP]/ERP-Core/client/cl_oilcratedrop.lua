local models = {"p_cargo_chute_s", "gr_prop_gr_rsply_crate04b", "titan", "s_m_m_pilot_02"} --What models the first one is the parachute and the second one is the brief case
...
RegisterNetEvent("OilcrateDrop")
AddEventHandler("OilcrateDrop", function(c)
    TriggerEvent("OilremoveCrate")
    ...
    TriggerServerEvent('Server:SoundToAll', 'cratedrop', 0.5);
    ...
    while GetEntityHeightAboveGround(activeCrate) > 27 do
        Wait(100)
    end
            ...
            if #(boxCoords - playerCoords) < 2.0 then
                if IsControlJustPressed(1, 51) then
                    if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
                        ...
                        if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
                            TriggerServerEvent('OilopenLootCrate', boxCoords, playerCoords)
                            StopSound(soundID)
...
RegisterNetEvent("OilremoveCrate")
AddEventHandler("OilremoveCrate", function()
    if activeCrate then
