...
function voteWeather(weatherType) 
	...
	if voteInProgress then
		if not voted then
			TriggerServerEvent("ERP:vote", weatherType)
			voted = true
...
function changeWeather()
	...
	TriggerServerEvent("ERP:setCurrentWeather",highestWeather)
end

RegisterNetEvent("ERP:voteFinished")
AddEventHandler("ERP:voteFinished", function(newWeather)
	currentWeather = newWeather
end)

RegisterNetEvent("ERP:startWeatherVote")
AddEventHandler("ERP:startWeatherVote", function()
	voteInProgress = true
...
RegisterNetEvent("ERP:voteStateChange")
AddEventHandler("ERP:voteStateChange",function(type)
	votesTable[type] = votesTable[type] + 1
end)

RegisterCommand("voteweather", function()
	TriggerServerEvent("ERP:tryStartWeatherVote")
end, false)
...
AddEventHandler('playerSpawned', function(spawn)
    TriggerServerEvent("ERP:getCurrentWeather")
