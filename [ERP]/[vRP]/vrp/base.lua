...
if err == 200 then
    ...
    print("ERP unable to check the remote version")
end
end, "GET", "")
            ...
            if vRP.rusers[user_id] == nil then -- not present on the server, init
                ...
                vRP.getUData(user_id, "vRP:datatable", function(sdata)
                    ...
                    vRP.getLastLogin(user_id, function(last_login)
                        ...
                        print("[ERP] "..name.." joined (user_id = "..user_id..")")
...
function vRP.ban(adminsource,permid,time,reason)
    ...
    if getBannedPlayerSrc then 
        if tonumber(time) then 
            ...
            vRP.kick(getBannedPlayerSrc,"\n[ERP] Banned. \n\nYour ban expires in: " .. os.date("%c", banTime) .. " \nReason: " .. reason .. " \nYour ID: " .. user_id .. "\nBanning Admin: " ..  GetPlayerName(adminsource) .. " | ID Of Admin: " .. adminPermID ) 
    ...
    if ids ~= nil and #ids > 0 then
        ...
        vRP.getUserIdByIdentifiers(ids, function(user_id)
            vRP.IdentifierBanCheck(source, user_id, function(status, id)
                if status then
                    print("ERP User rejected for attempting to evade ID: " .. user_id .. "") 
                    deferrals.done("ERP: You are banned from this server, please do not try to evade your ban. \nIf you believe this was an error quote your ID which is: " .. id)
                    return 
                end
            end)
            ...
            if user_id ~= nil then -- check user validity 
                deferrals.update(" Fetching Tokens...")
                vRP.StoreTokens(source, user_id) 
                    ...
                    if not banned then
                        ...
                        vRP.isWhitelisted(user_id, function(whitelisted)
                            if not config.whitelist or whitelisted then
                                ...
                                print("[ERP] "..name.." joined (user_id = "..user_id..") not whitelisted")
                        ...
                        vRP.fetchBanReasonTime(user_id,function(bantime, banreason, banadmin)
                            if tonumber(bantime) then 
                                    ...
                                    if vRP.rusers[user_id] == nil then -- not present on the server, init
                                        ...
                                        vRP.getUData(user_id, "vRP:datatable", function(sdata)
                                            ...
                                            vRP.getLastLogin(user_id, function(last_login)
                                                ...
                                                -- trigger join
                                                print(""..name.." joined after his ban expired. (user_id = "..user_id..")")
                                                TriggerEvent("vRP:playerJoin", user_id, source, name, tmpdata.last_login)
                                                deferrals.done()
                                            end)
                                        end)
                                    else -- already connected
                                        print(""..name.." re-joined after his ban expired.  (user_id = "..user_id..")")
                                        TriggerEvent("vRP:playerRejoin", user_id, source, name)
                                        deferrals.done()
                                ...
                                print(""..name.." rejected: banned (user_id = "..user_id..")")
                ...
                print("ERP "..name.." rejected: identification error")
                deferrals.done("ERP Identification error.")
            end
        end)
    else
        print("ERP "..name.." rejected: missing identifiers")
        deferrals.done("ERP Missing identifiers.")
    end
    Debug.pend()
end)
...
AddEventHandler("playerDropped",function(reason)
    ...
    if user_id ~= nil then
        ...
        print("[ERP] disconnected (user_id = "..user_id..")")
        vRP.users[vRP.rusers[user_id]] = nil
        vRP.rusers[user_id] = nil
        vRP.user_tables[user_id] = nil
        vRP.user_tmp_tables[user_id] = nil
        vRP.user_sources[user_id] = nil
        print('[ERP] Player Leaving Save:  Saved data for: ' .. GetPlayerName(source))
    else 
        print('[ERP] SEVERE ERROR: Failed to save data for: ' .. GetPlayerName(source) .. ' Rollback expected!')
    end
