...
AddEventHandler('es_bank:rob', function(robb)
	...
	if vRP.hasPermission({user_id,"police.armoury"}) then
		vRPclient.notify(player,{"~r~Police Officers can't rob stores."})
	else
		if #cops > 2 then
			if banks[robb] then
				...
				SetTimeout(180000, function()
					if(robbers[savedSource])then
						if(user_id)then
							vRP.giveMoney({user_id, math.random(50000,100000)})
			...
			vRPclient.notify(player,{"~r~There are not enough Police online."})
		end 
	end    
end)
