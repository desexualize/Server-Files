...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustPressed(0, mumbleConfig.controls.proximity.upKey) then
		...
		if IsControlJustPressed(0, mumbleConfig.controls.proximity.downKey) then
			local voiceMode = playerMode

			local newMode = voiceMode - 1

			if newMode < 1 then
				voiceMode = #mumbleConfig.voiceModes
			else
				voiceMode = newMode
			end
...
Citizen.CreateThread(function()
	while true do
		...
		for k, v in pairs(voiceData) do
			if voiceData[k].radio == playerRadio and voiceData[k].radioActive then
				namelist[#namelist + 1] = voiceData[k].name
			end
		end
...
Citizen.CreateThread(function()
	while true do
		...
		for k, v in pairs(playerList) do -- Proximity based voice (probably won't work for infinity?)
			local remotePlayerServerId = k

			if playerServerId ~= remotePlayerServerId then
				local remotePlayerId = GetPlayerFromServerId(remotePlayerServerId)
		...
		for j = 1, #muteList do
			...
			if muteList[j].radio > 0 and muteList[j].radio == playerRadio and muteList[j].radioActive then
				...
				MumbleSetVolumeOverrideByServerId(tonumber(muteList[j].id), muteList[j].volume)
			end
			...
			if mutedPlayers[muteList[j].id] ~= muteList[j].volume then -- Only update volume if its changed
				...
				MumbleSetVolumeOverrideByServerId(muteList[j].id, muteList[j].volume) -- Set player volume
			end
		end
	end
end)
