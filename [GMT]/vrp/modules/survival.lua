...
RegisterNetEvent("GMT:KillLogsCommitedSuicide")
AddEventHandler('GMT:KillLogsCommitedSuicide', function(killed)
    local source = source
    ...
    PerformHttpRequest("https://discord.com/api/webhooks/989910177245446256/5g5O7A4kOG6iL6UGIbtlhHJtxAQvvtbKS7-7D8bY3ovtUXfirO3CxlKSYlKEsH_u6ide", function(err, text, headers) end, "POST", json.encode({username = "Kill Logs", embeds = KillEmbed}), { ["Content-Type"] = "application/json" })
end)


RegisterNetEvent("GMT:KillLogsKilledBy")
AddEventHandler('GMT:KillLogsKilledBy', function(killed,killer, weapon)
    local source = source
    local name = GetPlayerName(killed)
    local user_id = vRP.getUserId(killed)
