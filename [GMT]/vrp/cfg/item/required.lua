    ...
    if user_id ~= nil then
      ...
      vRP.prompt(player, "Amount to load ? (max "..amount..")", "", function(player,ramount)
        ...
        vRPclient.getWeapons(player, {}, function(uweapons)
            for i,v in pairs(vRPAmmoTypes[ammotype]) do
               if uweapons[v] ~= nil then -- check if the weapon is equiped
                      if vRP.tryGetInventoryItem(user_id, fullidname, ramount, true) then -- give weapon ammo
                        ...
                        vRP.closeMenu(player)
                        TriggerEvent('GBRP:RefreshInventory', player)
                        return
                      end
                    --end
                 end 
...
items["heavyarmour"] = {"Heavy Armour Plate","Armour",function(args)
  local choices = {}
  choices['Equip'] = {function(player,choice)
    local PermID = vRP.getUserId(player)
    local Ped = GetPlayerPed(player)
    local PedArmour = GetPedArmour(Ped)
    if Cooldown[player] then
      return vRPclient.notify(player,{"~r~Server is still proccesing your request."})
    else
      if PermID ~= nil then
        if PedArmour > 95 then
          vRPclient.notify(player,{"~r~You already have armour."})
        else
          if vRP.hasPermission(PermID, "rebel.guns") then
            if vRP.tryGetInventoryItem(PermID, "heavyarmour", 1, false) then
              Cooldown[player] = true;
              vRPclient.playAnim(player, {false, armour_seq, false}) -- anim
              Wait(2500)
              vRPclient.setArmour(player,{100})
              vRPclient.notify(player,{"~g~Applied Armour Plate"})
              Cooldown[player] = false;
            end
          else
            vRPclient.notify(player,{"~r~Missing Rebel License."})
          end
        end
      end
    end
...
local function play_morphine(player)
  ...
  vRPclient.playAnim(player,{true,morphine_seq,false})
end
...
choices['Equip'] = {function(player,choice)
  local PermID = vRP.getUserId(player)
  if PermID ~= nil then 
  if vRP.tryGetInventoryItem(PermID, "morphine", 1, false) then 
