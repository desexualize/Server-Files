RMenu.Add('GMTPoliceMenu', 'main', RageUI.CreateMenu("","GMT Police",1250,100, "banners", "police"))
RMenu.Add('GMTPoliceMenu', 'objectmenu',  RageUI.CreateSubMenu(RMenu:Get("GMTPoliceMenu", "main")))
RMenu:Get('GMTPoliceMenu', 'main'):SetPosition(1300, 100)
RMenu:Get('GMTPoliceMenu', 'objectmenu'):SetPosition(1300, 100)
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('GMTPoliceMenu', 'main')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true}, function()
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
                RageUI.Button("Object Menu" , nil, { RightLabel = '→→→'}, true, function(Hovered, Active, Selected) end, RMenu:Get('GMTPoliceMenu', 'objectmenu'))
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('GMTPoliceMenu', 'objectmenu')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true}, function()
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('GMTPoliceMenu', 'speedzones')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true}, function()
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
                ...
                RageUI.Button("Delete Speedzone" , nil, {}, true, function(Hovered, Active, Selected) 
                    if Selected then 
                        TriggerServerEvent('GMT:RemoveZone')
...
RegisterCommand('pd', function()
  if IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then
    TriggerServerEvent('GMT:OpenPoliceMenu')
  end
end)
...
RegisterNetEvent("GMT:PoliceMenuOpened")
AddEventHandler("GMT:PoliceMenuOpened",function()
  RageUI.Visible(RMenu:Get('GMTPoliceMenu', 'main'), not RageUI.Visible(RMenu:Get('GMTPoliceMenu', 'main')))
end)
...
function createZone() 
  ...
  if speed == 0 then 
    ...
    return
      ...
      TriggerServerEvent('GMT:ActivateZone', message, speed, radius, x, y, z)
end

RegisterNetEvent('GMT:ZoneCreated')
AddEventHandler('GMT:ZoneCreated', function(speed, radius, x, y, z)
...
RegisterNetEvent('GMT:RemovedBlip')
AddEventHandler('GMT:RemovedBlip', function()
...
RegisterNetEvent("GMT:DragPlayer")
AddEventHandler('GMT:DragPlayer', function(pl)
    other = pl
    drag = not drag
end)
