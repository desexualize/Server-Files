...
AddEventHandler("GMT:GotGangData",function(i,j,k,PO)
    if i==nil then 
        PlayerIsInGang=false 
    else 
        PlayerIsInGang=true
...
AddEventHandler("GMT:disbandedGang",function()
    showGangUI=false
    showNoGangUI=true
    PlayerIsInGang=false
...
AddEventHandler("GMT:ForceRefreshData",function()
    TriggerServerEvent("GMT:GetGangData")
end)
RegisterNetEvent("GMT:InviteRecieved")
AddEventHandler("GMT:InviteRecieved",function(o,Q)
    GMTGangInvites[GMTGangInviteIndex]=Q
    GMTGangInviteIndex=GMTGangInviteIndex+1
    Notify(o)
end)
RegisterNetEvent("GMT:gangNameNotTaken")
AddEventHandler("GMT:gangNameNotTaken",function()
    showNoGangUI=false
    showGangUI=true
    PlayerIsInGang=true 
end)
function func_drawGangUI()
    if showNoGangUI then 
        ...
        DrawAdvancedText(0.558,0.303,0.005,0.0028,0.539,"GMT Gangs",255,255,255,255,7,0)
        ...
        for p,q in pairs(GMTGangInvites)do 
        ...
        if CursorInArea(0.435,0.51,0.415,0.46)then 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if selectedGangInvite~=nil then 
                    ...
                    TriggerServerEvent("GMT:addUserToGang",selectedGangInvite,perm1s)
                    GMTGangInvites={}
                    showNoGangUI=false
                    GMTGangInviteIndex=0
                    showGangUI=true
    ...
    if showFundsGangUI then 
        ...
        DrawAdvancedText(0.591,0.378,0.005,0.0028,0.48,"GMT gang - funds",255,255,255,255,7,0)
        ...
        if CursorInArea(0.4083,0.4718,0.5490,0.5999)then 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if amount~=nil then 
                    if closeToLegion()then 
                        TriggerServerEvent("GMT:depositGangBalance",amount)
                    end 
                else 
    ...
    if showMembersGangUI then 
        ...
        DrawAdvancedText(0.591,0.312,0.005,0.0028,0.48,"GMT gang - members",255,255,255,255,7,0)
                ...
                if selectedMember~=nil then 
                    if gangPermission>=3 then 
                        if YesNoConfirm()then 
                            TriggerServerEvent("GMT:kickMemberFromGang",gangID,selectedMember)
                        end 
                    else 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                ...
                if s~=nil then 
                    if gangPermission>=2 then 
                        TriggerServerEvent("GMT:InviteUserToGang",gangID,s)
                    else 
    ...
    if showLogsGangUI then 
        DrawRect(0.501,0.525,0.421,0.387,0,0,0,150)
        DrawRect(0.501,0.308,0.421,0.047,161,0,0,248)
        DrawAdvancedText(0.591,0.312,0.005,0.0028,0.48,"GMT gang - logs",255,255,255,255,7,0)
    ...
    if showSettingsGangUI then 
        DrawRect(0.501,0.525,0.421,0.387,0,0,0,150)
        DrawRect(0.501,0.308,0.421,0.047,161,0,0,248)
        DrawAdvancedText(0.591,0.312,0.005,0.0028,0.48,"GMT gang - settings",255,255,255,255,7,0)
        ...
        if CursorInArea(0.3187,0.3937,0.4712,0.5462)then 
            ...
            if IsControlJustPressed(1,329)or IsDisabledControlJustPressed(1,329)then 
                PlaySound(-1,"SELECT","HUD_FRONTEND_DEFAULT_SOUNDSET",0,0,1)
                if YesNoConfirm()then 
                    TriggerServerEvent("GMT:memberLeaveGang",gangID)
                    PlayerIsInGang=false
                    showGangUI=false
                    showNoGangUI=true
                    showSettingsGangUI=false
    ...
    if showTurfsGangUI then 
        DrawRect(0.501,0.525,0.421,0.387,0,0,0,150)
        DrawRect(0.501,0.308,0.421,0.047,161,0,0,248)
        DrawAdvancedText(0.591,0.312,0.005,0.0028,0.48,"GMT gang - Turfs",255,255,255,255,7,0)
    ...
    if showGangUI then 
        ...
        DrawAdvancedText(0.591,0.399,0.005,0.0028,0.51,"GMT Gangs",255,255,255,255,7,0)
...
RegisterNetEvent("GMT:Notify")
AddEventHandler("GMT:Notify",function(u)
    Notify(u)
end)
function closeToLegion()
    ...
    if w<10.0 or x<10.0 then 
        ...
        Notify("~r~You must be near Halifax to deposit/withdraw")
        return false 
    end 
end
