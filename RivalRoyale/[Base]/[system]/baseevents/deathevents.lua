    ...
    while true do
        ...
        if NetworkIsPlayerActive(player) then
            ...
            if IsPedFatallyInjured(ped) and not isDead then
                ...
                if killer ~= ped and killerid ~= nil and
                    NetworkIsPlayerActive(killerid) then
            ...
            if not hasBeenDead and diedAt ~= nil and diedAt > 0 then
                TriggerEvent('baseevents:onPlayerWasted',
                             {table.unpack(GetEntityCoords(ped))})
                TriggerServerEvent('baseevents:onPlayerWasted',
                                   {table.unpack(GetEntityCoords(ped))})
