...
function NovaDrugsServer.DiamondGather()
...
function NovaDrugsServer.DiamondCanProcess()
  local user_id = vRP.getUserId({source})
  return vRP.hasPermission({user_id, "diamond.job"}),
  vRP.getInventoryItemAmount({user_id, 'crystal'}) >= 5
end

function NovaDrugsServer.DiamondDoneProcessing()
  local user_id = vRP.getUserId({source})
...
function NovaDrugsServer.SellDiamondJob(amount)
 local user_id = vRP.getUserId({source})
