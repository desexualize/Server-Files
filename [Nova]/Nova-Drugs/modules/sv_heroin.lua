function NovaDrugsServer.HeroinGather()
...
function NovaDrugsServer.CanProcessHeroin()
    local user_id = vRP.getUserId({source})
...
function NovaDrugsServer.ProcessHeroin()
    local user_id = vRP.getUserId({source})
...
function NovaDrugsServer.SellHeroin(amount)
    local user_id = vRP.getUserId({source})
    if vRP.tryGetInventoryItem({user_id,'heroin', 1}) then
      ...
      if user_id ~= nil then
        ...
        local price = 40000 * tonumber(amount)
