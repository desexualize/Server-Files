...
Citizen.CreateThread(function()
  while true do
    ...
    if IsPlayerNearCoords(vector3(Drugs.ScrapJob.Gather.x,Drugs.ScrapJob.Gather.y,Drugs.ScrapJob.Gather.z), 40.0) then
      BeginTextCommandPrint("STRING")
      AddTextComponentSubstringPlayerName("Press [E] to Fix Scrap")
          ...
          while InRangeGather do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              if not pedinveh then
                ...
                NovaDrugsServer.ScrapJobGather ()
                Action = false
              else
    ...
    if IsPlayerNearCoords(vector3(Drugs.ScrapJob.Sell.x,Drugs.ScrapJob.Sell.y,Drugs.ScrapJob.Sell.z), 100.0) then
      if not CanSeeMarker then
        CanSeeMarker = true
        Citizen.CreateThread(function()
          while CanSeeMarker do
            Citizen.Wait(0)
            -- Draw3DText(Drugs.ScrapJob.Sell.x,Drugs.ScrapJob.Sell.y,Drugs.ScrapJob.Sell.z, "Sell ScrapJob")

            DrawMarker(2,  Drugs.ScrapJob.Sell.x,Drugs.ScrapJob.Sell.y,Drugs.ScrapJob.Sell.z+1 - 0.98, 0, 0, 0, 0, 0, 0, 0.5, 0.5, 0.5, 207, 145, 0, 150, true, true, 0, 0, 0, 0, 0)
...
Citizen.CreateThread(function() 
    while true do
            ...
            local v1 = vector3(Drugs.ScrapJob.Sell.x,Drugs.ScrapJob.Sell.y,Drugs.ScrapJob.Sell.z)
...
RMenu.Add('ScrapJobMenu', 'ScrapJob Seller', RageUI.CreateMenu("", "~o~Scrap Job Seller",1300, 50, "scrapjob", "scrapjob"))
...
RageUI.CreateWhile(1.0, RMenu:Get('ScrapJobMenu', 'ScrapJob Seller'):SetPosition(1300, 50), nil, function()
  RageUI.IsVisible(RMenu:Get('ScrapJobMenu', 'ScrapJob Seller'), true, false, true, function()
    ...
    RageUI.Button("Sell Scrap" , nil, {}, true, function(Hovered, Active, Selected)
        if Selected then
...
function sellingScrapJobUnits()
    NovaDrugsServer.SellScrapJob({tonumber(1)})
    return false
end
