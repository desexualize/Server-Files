...
Citizen.CreateThread(function()
  while true do
    ...
    if IsPlayerNearCoords(vector3(Drugs.Weed.Gather.x,Drugs.Weed.Gather.y,Drugs.Weed.Gather.z), 15.0) then
      ...
      AddTextComponentSubstringPlayerName("Press [E] to gather Cannabis Sativa")
    ...
    if IsPlayerNearCoords(vector3(Drugs.Weed.Process.x,Drugs.Weed.Process.y,Drugs.Weed.Process.z), 25) then
      BeginTextCommandPrint("STRING")
      AddTextComponentSubstringPlayerName("Press [E] to process Weed")
      EndTextCommandPrint(1000, 1)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              NovaDrugsServer.CanProcessWeed({}, function(canProcess, ketAmount)
                if canProcess and ketAmount then

          


                  NovaDrugsServer.ProcessWeed()
      ...
      if not CanSeeMarker then
        ...
        Citizen.CreateThread(function()
          while CanSeeMarker do
            Citizen.Wait(0)
            -- Draw3DText(Drugs.Weed.Sell.x,Drugs.Weed.Sell.y,Drugs.Weed.Sell.z, "Sell Weed")
...
RMenu.Add('WeedMenu', 'Weed Seller', RageUI.CreateMenu("", "~g~Weed Seller",1300, 50, "Weed", "Weed"))
...
function sellingWeedUnits()
    NovaDrugsServer.SellWeed({tonumber(1)})
    return false
end
