...
Citizen.CreateThread(function()
  while true do
    ...
    if IsPlayerNearCoords(vector3(Drugs.Diamond.Gather.x,Drugs.Diamond.Gather.y,Drugs.Diamond.Gather.z), 40.0) then
      ...
      AddTextComponentSubstringPlayerName("Press [E] to collect Diamond Crystals")
    ...
    if IsPlayerNearCoords(vector3(Drugs.Diamond.Process.x,Drugs.Diamond.Process.y,Drugs.Diamond.Process.z), 60.0) then
      BeginTextCommandPrint("STRING")
      AddTextComponentSubstringPlayerName("Press [E] to process Diamonds")
      EndTextCommandPrint(1000, 1)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              NovaDrugsServer.DiamondCanProcess({}, function(canProcess, ketAmount)
                if canProcess and ketAmount then
                  ...
                  NovaDrugsServer.DiamondDoneProcessing()
...
RMenu.Add('DiamondMenu', 'Diamond Seller', RageUI.CreateMenu("", "~b~Diamond Seller", 1300, 50, "Diamond", "Diamond"))
...
function sellingDiamondUnits()
     NovaDrugsServer.SellDiamondJob({tonumber(1)})
    return false
end
