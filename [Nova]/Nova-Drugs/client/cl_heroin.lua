...
Citizen.CreateThread(function()
  while true do
    ...
    if IsPlayerNearCoords(vector3(Drugs.Heroin.Gather.x,Drugs.Heroin.Gather.y,Drugs.Heroin.Gather.z), 20.0) then
      ...
      AddTextComponentSubstringPlayerName("Press [E] to gather Opium")
    ...
    if IsPlayerNearCoords(vector3(Drugs.Heroin.Process.x,Drugs.Heroin.Process.y,Drugs.Heroin.Process.z), 20.0) then
      BeginTextCommandPrint("STRING")
      AddTextComponentSubstringPlayerName("Press [E] to process Heroin.")
      EndTextCommandPrint(1000, 1)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              NovaDrugsServer.CanProcessHeroin({}, function(canProcess, ketAmount)
...
RMenu.Add('HeroinMenu', 'Heroin Seller', RageUI.CreateMenu("", "~r~Heroin Seller",1300, 50, "heroin", "heroin"))
...
function sellingHeroinUnits()
    NovaDrugsServer.SellHeroin({tonumber(1)})
    return false
end
