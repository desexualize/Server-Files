...
Citizen.CreateThread(function()
  while true do
    ...
    if IsPlayerNearCoords(vector3(Drugs.LSD.Gather.x,Drugs.LSD.Gather.y,Drugs.LSD.Gather.z), 30.0) then
      ...
      AddTextComponentSubstringPlayerName("Press [E] to gather Lysergic Acid")
    ...
    if IsPlayerNearCoords(vector3(Drugs.LSD.Process.x,Drugs.LSD.Process.y,Drugs.LSD.Process.z), 30.0) then
      BeginTextCommandPrint("STRING")
      AddTextComponentSubstringPlayerName("Press [E] to process LSD")
      EndTextCommandPrint(1000, 1)
        ...
        Citizen.CreateThread(function()
          while InRangeProcess do
            ...
            if IsControlJustReleased(0, 51)  then
              ...
              exports.rprogress:Start("", 5000)
              NovaDrugsServer.CanProcessLSD({}, function(canProcess, ketAmount)
...
RMenu.Add('LSDMenu', 'LSD Seller', RageUI.CreateMenu("", "~p~LSD Seller",1300, 50, "LSD", "LSD"))
...
function sellingLSDUnits()
    NovaDrugsServer.SellLSD({tonumber(1)})
    return false
end
