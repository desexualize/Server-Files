...
RegisterCommand('els', function(source, args)
    ...
    if args[1] == 'version' then
        PerformHttpRequest(latestVersionPath, function(err, response, headers)
            ...
            if source > 0 then
                return notify(source, "~b~ELS~s~~n~Version " .. curVerCol(thisVersion) .. curVersion)
            end
    ...
    elseif not args[1] or args[1] == 'help' then
        TriggerClientEvent("els:notify", source, "~b~ELS~s~~n~Version " .. curVerCol(latestVersion) .. curVersion)
...
AddEventHandler("els:playerSpawned", function()
    ...
    if curVersion < latestVersion then
        TriggerClientEvent("els:notify", source, "~b~ELS-FiveM~s~~n~Outdated version! Please update as soon as possible.")
