        ...
        vRP.getSData(
            {
                ...
                function(cdata)
                    ...
                    local maxVehKg =
                        Inventory.vehicle_chest_weights[spawnCode] or Inventory.default_vehicle_chest_weight
                    TriggerClientEvent(
...
RegisterNetEvent("Nova:LockPick")
AddEventHandler(
    "Nova:LockPick",
    function()
        ...
        if vRP.tryGetInventoryItem({user_id, "lockpick", 1, true}) then
            TriggerClientEvent("Nova:whatIsThis", source)
        end
    end
)
                            ...
                            function(cdata)
                                ...
                                if cdata[itemId] and cdata[itemId].amount >= 1 then
                                    ...
                                    if weightCalculation <= vRP.getInventoryMaxWeight({UserId}) then
                                        ...
                                        local maxVehKg =
                                            Inventory.vehicle_chest_weights[inventoryInfo] or
                                            Inventory.default_vehicle_chest_weight
                                        TriggerClientEvent(
                                            "Nova:SendSecondaryInventoryData",
                                            source,
                                            FormattedInventoryData,
...
AddEventHandler(
    "Nova:MoveItemAll",
    function(inventoryType, itemId, inventoryInfo, vehid)
        local source = source
        local UserId = vRP.getUserId({source})
        local data = vRP.getUserDataTable({UserId})
        if not itemId then
    ...
    function(netid)
        ...
        vRPclient.isInComa(
            ...
            function(in_coma)
                if not in_coma then
                    if
                        LootBagEntities[netid] and not LootBagEntities[netid][3] and
                            #(GetEntityCoords(LootBagEntities[netid][1]) - GetEntityCoords(GetPlayerPed(source))) < 5.0
                     then
                        ...
                        if user_id ~= nil then
                            ...
                            vRPclient.notify(
                                source,
                                {"~g~You have opened " .. LootBagEntities[netid].name .. "'s lootbag"}
                            )
                        end
                    else
