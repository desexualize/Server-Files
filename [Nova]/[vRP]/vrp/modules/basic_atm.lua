...
RegisterNetEvent("Nova:Withdraw")
AddEventHandler("Nova:Withdraw", function(amount)
    local source = source
                ...
                if amount > 0 then
                    ...
                    if user_id ~= nil then
                        if vRP.tryWithdraw(user_id, amount) then
                            ...
                            webhook =
                                "https://discord.com/api/webhooks/1012507078625071114/SAWX4klhws6_S8HKBPjUIs71vhwvsKD8ZV0QNCAWPRckgsT4K74BnKw2dD5oAHV0r8GJ"
                            PerformHttpRequest(
                                webhook,
...
RegisterNetEvent("Nova:Deposit")
AddEventHandler("Nova:Deposit", function(amount)
    local source = source
    amount = parseInt(amount)
    if onesync ~= "off" then
        --Onesync allows extra security behind events should enable it if it's not already.
                ...
                if amount > 0 then
                    ...
                    if user_id ~= nil then
                        if vRP.tryDeposit(user_id, amount) then
                            ...
                            webhook =
                                "https://discord.com/api/webhooks/1012507459782443019/SS9uVos0zo188DVlHqRX1wqXU6PuscukwLO5DZM5ffv_sNzKUgFGKDlhDE3aDum7i79n"
                            PerformHttpRequest(
                                webhook,
                                function(err, text, headers)
                                end,
...
RegisterNetEvent('Nova:DepositAll')
AddEventHandler('Nova:DepositAll', function()
    local source = source  
    local ped = GetPlayerPed(source)
    local playerCoords = GetEntityCoords(ped)
    for i, v in pairs(cfg.atms) do
        ...
        if #(playerCoords - coords) <= 5.0 then
            ...
            if vRP.tryDeposit(user_id, money) then 
                vRPclient.notify(source,{lang.atm.deposit.deposited({money})})
                webhook = "https://discordapp.com/api/webhooks/1012507078625071114/SAWX4klhws6_S8HKBPjUIs71vhwvsKD8ZV0QNCAWPRckgsT4K74BnKw2dD5oAHV0r8GJ"
                ...
                vRPclient.notify(source,{"~r~You Dont Have Any Money."})
            end
        end
    end
end)

RegisterNetEvent('Nova:WithdrawAll')
AddEventHandler('Nova:WithdrawAll', function()
local source = source
local ped = GetPlayerPed(source)
local playerCoords = GetEntityCoords(ped)
user_id = vRP.getUserId(source)
amount = vRP.getBankMoney(user_id)
for i, v in pairs(cfg.atms) do
    local coords = vec3(v[1], v[2], v[3])
    if #(playerCoords - coords) <= 5.0 then
...
local function atm_enter()
    ...
    if user_id ~= nil then
        atm_menu[lang.atm.info.title()] = {
            ...
            lang.atm.info.bank({vRP.getBankMoney(user_id)})
        }
