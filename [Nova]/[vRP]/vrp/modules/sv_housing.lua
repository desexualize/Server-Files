local Housing = module('vrp', 'cfg/cfg_homes')
ownedGaffs = {}

--SQL
MySQL.Create_Command("vRP/get_address","SELECT home, number FROM vrp_user_homes WHERE user_id = @user_id")
MySQL.Create_Command("vRP/set_address", "REPLACE INTO vrp_user_homes(user_id,home,number) VALUES(@user_id,@home,@number)")
MySQL.Create_Command("vRP/get_home_owner","SELECT user_id FROM vrp_user_homes WHERE home = @home AND number = @number")
MySQL.Create_Command("vRP/rm_address","DELETE FROM vrp_user_homes WHERE user_id = @user_id AND home = @home")
...
AddEventHandler('Housing:getHouses', function()
    ...
    exports.oxmysql:query("SELECT * FROM `vrp_user_homes` WHERE user_id = @user_id", {user_id = user_id}, function(result)
        if result ~= nil then 
...
function getUserAddress(user_id, cbr)
    ...
    MySQL.Sync_Query("vRP/get_address", {user_id = user_id}, function(rows, affected)
        task({rows[1]})
    end)
end
  
function setUserAddress(user_id, home, number)
    MySQL.Execute_Command("vRP/set_address", {user_id = user_id, home = home, number = number})
end
  
function removeUserAddress(user_id, home)
    MySQL.Execute_Command("vRP/rm_address", {user_id = user_id, home = home})
end

function getUserByAddress(home, number, cbr)
    local task = Task(cbr)
  
    MySQL.Sync_Query("vRP/get_home_owner", {home = home, number = number}, function(rows, affected)
        if #rows > 0 then
            task({rows[1].user_id})
        else
            task()
                ...
                if noowner == nil then
                    getUserAddress(user_id, function(address) -- check if user already has a home
                            if vRP.tryFullPayment({user_id,v.buy_price}) then --try payment
                                ...
                                local webhook = 'https://discord.com/api/webhooks/1018797094544363530/J9lowB9vCyfTdyQrzvMz6r5yI1EG_3fQEGaCikW1vLStJThSuaobeYvGfnCE5SXdWGSj'
                                local embed = {
                                    {
                                        ["color"] = "16777215",
                                        ["title"] = "House Logs",
                                ...
                                PerformHttpRequest(webhook, function (err, text, headers) end, 'POST', json.encode({username = 'Nova', embeds = embed}), { ['Content-Type'] = 'application/json' })
                            else
