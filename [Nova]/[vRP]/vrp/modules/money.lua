...
MySQL.Create_Command("vRP/get_money", "SELECT wallet,bank FROM vrp_user_moneys WHERE user_id = @user_id")
MySQL.Create_Command(
  "vRP/set_money",
  "UPDATE vrp_user_moneys SET wallet = @wallet, bank = @bank WHERE user_id = @user_id"
)
...
AddEventHandler(
  ...
  function(user_id, source)
    ...
    if tmp and tmp.wallet ~= nil and tmp.bank ~= nil then
      MySQL.Execute_Command("vRP/set_money", {user_id = user_id, wallet = tmp.wallet, bank = tmp.bank})
    end
  end
)
  ...
  function()
    for k, v in pairs(vRP.user_tmp_tables) do
      if v.wallet ~= nil and v.bank ~= nil then
        MySQL.Execute_Command("vRP/set_money", {user_id = k, wallet = v.wallet, bank = v.bank})
      end
    end
  end
)
