...
function vRP.defInventoryItem(idname, name, description, choices, weight)
  ...
  item.ch_trash = function(player, choice)
    ...
    if user_id ~= nil then
      -- prompt number
      --   TriggerClientEvent('Nova:ToggleNUIFocus', player, false)
      vRP.prompt(
        player,
        ...
        function(player, amount)
          ...
          if vRP.tryGetInventoryItem(user_id, idname, amount, false) then
            --   TriggerClientEvent('Nova:ToggleNUIFocus', player, true)
            TriggerEvent("Nova:RefreshInventory", vRP.getUserSource(user_id))
              ...
              function(player, amount)
                ...
                if new_weight <= vRP.getInventoryMaxWeight(nuser_id) then
                  if vRP.tryGetInventoryItem(user_id, idname, amount, true) then
                    ...
                    TriggerEvent("Nova:RefreshInventory", player)
                    TriggerEvent("Nova:RefreshInventory", nplayer)
...
function vRP.RunTrashTask(source, itemName)
  ...
  if choices["Trash"] then
    ...
    print(
      "[^7JamesUKInventory]^1: Invalid item removed from inventory space. Usually caused by spawned in staff items. User item from: " ..
        user_id .. " Item Name: " .. itemName
    )
  end
      ...
      menudata[
          '<div class="dprogressbar" data-value="' ..
            string.format("%.2f", weight / max_weight) ..
              '" data-color="hsl(' ..
                hue ..
                  ',100%,50%)" data-bgcolor="hsl(' ..
                    hue .. ',100%,25%)" style="height: 12px; border: 3px solid black;"></div>'
        ] = {function()
        end, lang.inventory.info_weight({string.format("%.2f", weight), max_weight})}
      local kitems = {}
...
local choices = {}
choices[lang.inventory.title()] = {function(player, choice)
    vRP.openInventory(player)
  end, lang.inventory.description()}
        ...
        vRP.getSData(
          ...
          function(cdata)
            ...
            local cb_take = function(idname)
              ...
              vRP.prompt(
                source,
                lang.inventory.chest.take.prompt({citem.amount}),
                "",
                function(player, amount)
