...
AddEventHandler(
    ...
    function(netid)
        local source = source
        if
            MoneydropEntities[netid] and not MoneydropEntities[netid][3] and
                #(GetEntityCoords(MoneydropEntities[netid][1]) - GetEntityCoords(GetPlayerPed(source))) < 10.0
         then
            MoneydropEntities[netid][3] = true
            local user_id = vRP.getUserId(source)
            if user_id ~= nil then
                TriggerClientEvent("Nova:MoneyNotInBag", source)
