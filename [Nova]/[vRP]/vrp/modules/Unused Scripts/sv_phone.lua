...
RegisterServerEvent("Nova:AskID")
AddEventHandler(
  "Nova:AskID",
  function()
    local player = source
                        ...
                        function(business)
                          ...
                          vRP.getUserAddress(
                            ...
                            function(address)
                              ...
                              local content =
                                lang.police.identity.info(
                                {name, firstname, age, registration, phone, bname, bcapital, home, number}
                              )
                              vRPclient.setDiv(
                                player,
                                {
...
RegisterServerEvent("Nova:GiveMoney")
AddEventHandler(
  "Nova:GiveMoney",
  function(amount)
    local player = source
...
RegisterNetEvent("Nova:TrunkOpened")
AddEventHandler(
  "Nova:TrunkOpened",
  function()
...
RegisterNetEvent("Nova:SearchPlr")
AddEventHandler(
  "Nova:SearchPlr",
  function()
    player = source
...
RegisterServerEvent("Nova:SendPayment")
AddEventHandler(
  "Nova:SendPayment",
  function(playerid, price)
    local source = source
      ...
      if userid == recieverid then
        ...
        if vRP.tryBankPayment(userid, tonumber(price)) then
          vRPclient.notify(
            source,
            {
              "~g~Successfully transfered: ~w~" ..
                price ..
                  " Token(s) ~g~to ~w~" .. vRP.getPlayerName(reciever) .. " ~r~ ~n~ ~n~[ID: ~w~" .. playerid .. " ~r~]"
            }
          )
          ...
          vRPclient.notify(
            reciever,
            {
              "~g~You have recieved: ~w~" ..
                price ..
                  " Token(s)~g~ from ~w~" .. vRP.getPlayerName(source) .. " ~r~ ~n~ ~n~[ID: ~w~" .. userid .. " ~r~]"
            }
          )
          TriggerClientEvent("Nova:PlaySound", reciever, 1)

          webhook =
            "https://discord.com/api/webhooks/1012508397603672184/bpFS84hTb8VzNu20V1VDLoWPqgbG-YS4jKMziPpssvPYVO33p2ZP-OckLzbHxXashwDV"
          PerformHttpRequest(
            webhook,
                      ...
                      GetPlayerName(source) ..
                        "' [" ..
                          vRP.getUserId(source) ..
                            "] **MPayed **" .. vRP.getPlayerName(reciever) .. "[ " .. vRP.getUserId(reciever) .. "]**",
                    ["footer"] = {
