...
AddEventHandler("vRP:playerSpawn",function(user_id, source, first_spawn)
    ...
    if first_spawn then -- first spawn
        ...
        if data.customization ~= nil then
            vRPclient.setCustomization(source,{data.customization},function()
                if data.weapons ~= nil then
                    ...
                    if data.health ~= nil then -- set health
                        vRPclient.setHealth(source, {data.health})SetTimeout(5000,function()
            ...
            vRPclient.spawnAnim(source, {data.customization, data.position, data.coma})
        else
...
AddEventHandler("Nova:StoreWeaponsRequest",function(source)
    local player = source
    local user_id = vRP.getUserId(player)
    vRPclient.getWeapons(player,{},function(weapons)
        if not isStoring[player] then
