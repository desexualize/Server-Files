...
AddEventHandler("LSC:finished", function(veh)
	...
	local user_id = vRP.getUserId(source)
	if user_id then
		MySQL.Execute_Command("vRPls/update_vehicle_modifications", {user_id = user_id, vehicle = veh.model, modifications = json.encode({color = veh.color, extraColor = veh.extracolor, neon = veh.neon, neonColor = veh.neoncolor, xenonColor = veh.xenoncolor, smokeColor = veh.smokecolor, wheelType = veh.wheeltype, bulletProofTyres = veh.bulletProofTyres, windowTint = veh.windowtint, plateIndex = veh.plateindex, mods = veh.mods})})
	end
end)
...
AddEventHandler("LSC:applyModifications", function (model, vehicle)
	...
	if model and vehicle and user_id then
		local rows = MySQL.Sync_Query("vRPls/get_vehicle_modifications", {user_id = user_id, vehicle = model}, function(rows, affected) 
			if rows ~= nil then 
				if #rows > 0 then
