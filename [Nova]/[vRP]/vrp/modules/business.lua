...
MySQL.Create_Command("vRP/create_business","INSERT IGNORE INTO vrp_user_business(user_id,name,description,capital,laundered,reset_timestamp) VALUES(@user_id,@name,'',@capital,0,@time)")
MySQL.Create_Command("vRP/delete_business","DELETE FROM vrp_user_business WHERE user_id = @user_id")
MySQL.Create_Command("vRP/get_business","SELECT name,description,capital,laundered,reset_timestamp FROM vrp_user_business WHERE user_id = @user_id")
MySQL.Create_Command("vRP/add_capital","UPDATE vrp_user_business SET capital = capital + @capital WHERE user_id = @user_id")
MySQL.Create_Command("vRP/add_laundered","UPDATE vrp_user_business SET laundered = laundered + @laundered WHERE user_id = @user_id")
MySQL.Create_Command("vRP/get_business_page","SELECT user_id,name,description,capital FROM vrp_user_business ORDER BY capital DESC LIMIT @b,@n")
MySQL.Create_Command("vRP/reset_transfer","UPDATE vrp_user_business SET laundered = 0, reset_timestamp = @time WHERE user_id = @user_id")
...
function vRP.getUserBusiness(user_id, cbr)
  ...
  if user_id ~= nil then
    MySQL.Sync_Query("vRP/get_business", {user_id = user_id}, function(rows, affected)
      ...
      if business and os.time() >= business.reset_timestamp+cfg.transfer_reset_interval*60 then
        MySQL.Execute_Command("vRP/reset_transfer", {user_id = user_id, time = os.time()})
...
function vRP.closeBusiness(user_id)
  MySQL.Execute_Command("vRP/delete_business", {user_id = user_id})
end
...
local function open_business_directory(player,page) -- open business directory with pagination system
  ...
  MySQL.Sync_Query("vRP/get_business_page", {b = page*10, n = 10}, function(rows, affected)
    local count = 0
          ...
          vRP.prompt(player,lang.business.addcapital.prompt(),"",function(player,amount)
            ...
            if amount > 0 then
              if vRP.tryPayment(user_id,amount) then
                MySQL.Execute_Command("vRP/add_capital", {user_id = user_id, capital = amount})
            ...
            vRP.prompt(player,lang.business.launder.prompt({launder_left}),""..launder_left,function(player,amount)
              ...
              if amount > 0 and amount <= launder_left then
                if vRP.tryGetInventoryItem(user_id,"dirty_money",amount,false) then
                  ...
                  MySQL.Execute_Command("vRP/add_laundered", {user_id = user_id, laundered = amount})
              ...
              vRP.prompt(player,lang.business.open.prompt_capital({cfg.minimum_capital}),""..cfg.minimum_capital,function(player,capital)
                ...
                if capital >= cfg.minimum_capital then
                  if vRP.tryPayment(user_id,capital) then
                    MySQL.Execute_Command("vRP/create_business", {
                      user_id = user_id,
                      name = name,
                      capital = capital,
                      time = os.time()
