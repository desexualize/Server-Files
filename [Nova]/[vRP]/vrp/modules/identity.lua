...
-- init sql
MySQL.Create_Command("vRP/get_user_identity","SELECT * FROM vrp_user_identities WHERE user_id = @user_id")
MySQL.Create_Command("vRP/init_user_identity","INSERT IGNORE INTO vrp_user_identities(user_id,registration,phone,firstname,name,age) VALUES(@user_id,@registration,@phone,@firstname,@name,@age)")
MySQL.Create_Command("vRP/update_user_identity","UPDATE vrp_user_identities SET firstname = @firstname, name = @name, age = @age, registration = @registration, phone = @phone WHERE user_id = @user_id")
MySQL.Create_Command("vRP/get_userbyreg","SELECT user_id FROM vrp_user_identities WHERE registration = @registration")
MySQL.Create_Command("vRP/get_userbyphone","SELECT user_id FROM vrp_user_identities WHERE phone = @phone")
...
function vRP.getUserIdentity(user_id, cbr)
    local task = Task(cbr)
    if cbr then 
        MySQL.Sync_Query("vRP/get_user_identity", {user_id = user_id}, function(rows, affected)
            if #rows > 0 then 
              task({rows[1]})
            else 
               task({})
...
function vRP.getUserByRegistration(registration, cbr)
  ...
  MySQL.Sync_Query("vRP/get_userbyreg", {registration = registration or ""}, function(rows, affected)
    if #rows > 0 then
      task({rows[1].user_id})
    else
      task()
...
function vRP.getUserByPhone(phone, cbr)
  ...
  MySQL.Sync_Query("vRP/get_userbyphone", {phone = phone or ""}, function(rows, affected)
    if #rows > 0 then
      task({rows[1].user_id})
    else
      task()
    ...
    if identity == nil then
      vRP.generateRegistrationNumber(function(registration)
        vRP.generatePhoneNumber(function(phone)
          MySQL.Execute_Command("vRP/init_user_identity", {
            user_id = user_id,
                ...
                if vRP.tryPayment(user_id,cfg.new_identity_cost) then
                  vRP.generateRegistrationNumber(function(registration)
                    vRP.generatePhoneNumber(function(phone)
                      ...
                      MySQL.Execute_Command("vRP/update_user_identity", {
                        user_id = user_id,
                        firstname = firstname,
                        name = name,
                        age = age,
