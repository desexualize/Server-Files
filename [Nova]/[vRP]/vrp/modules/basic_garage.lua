...
-- -- vehicle db
local lang = vRP.lang
local cfg = module('Nova-Cars', "garages")
local cfg_inventory = module("cfg/inventory")
local vehicle_groups = cfg.garage_types
local limit = cfg.limit or 100000000
MySQL.Create_Command(
    "vRP/add_vehicle",
    "INSERT IGNORE INTO vrp_user_vehicles(user_id,vehicle,vehicle_plate) VALUES(@user_id,@vehicle,@registration)"
)
MySQL.Create_Command(
    "vRP/remove_vehicle",
...
MySQL.Create_Command(
    "vRP/get_vehicles",
    "SELECT vehicle, rentedtime FROM vrp_user_vehicles WHERE user_id = @user_id AND rented = 0"
)
MySQL.Create_Command(
    "vRP/get_rented_vehicles_in",
    "SELECT vehicle, rentedtime, user_id FROM vrp_user_vehicles WHERE user_id = @user_id AND rented = 1"
)
MySQL.Create_Command(
    "vRP/get_rented_vehicles_out",
    "SELECT vehicle, rentedtime, user_id FROM vrp_user_vehicles WHERE rentedid = @user_id AND rented = 1"
)
MySQL.Create_Command(
    "vRP/get_vehicle",
...
MySQL.Create_Command(
    "vRP/check_rented",
    "SELECT * FROM vrp_user_vehicles WHERE user_id = @user_id AND vehicle = @vehicle AND rented = 1"
)
MySQL.Create_Command(
    "vRP/sell_vehicle_player",
    "UPDATE vrp_user_vehicles SET user_id = @user_id, vehicle_plate = @registration WHERE user_id = @oldUser AND vehicle = @vehicle"
)
MySQL.Create_Command(
    "vRP/rentedupdate",
    "UPDATE vrp_user_vehicles SET user_id = @id, rented = @rented, rentedid = @rentedid, rentedtime = @rentedunix WHERE user_id = @user_id AND vehicle = @veh"
)
MySQL.Create_Command("vRP/fetch_rented_vehs", "SELECT * FROM vrp_user_vehicles WHERE rented = 1")
            ...
            for a, z in pairs(v) do
                if a ~= "_config" and a == vehicle then
                    if vRP.tryFullPayment(user_id, z[2]) then
                        ...
                        vRPclient.notify(
                            source,
                            {"~r~You do not have enough money to purchase this vehicle! It costs: $" .. z[2]}
                        )
                ...
                function(pvehicles)
                    MySQL.Sync_Query(
                        ...
                        function(pveh)
                            ...
                            MySQL.Execute_Command("vRP/remove_vehicle", {user_id = user_id, vehicle = vehicle})
                            TriggerClientEvent("vRP:CloseGarage", source)
                        end
                    )
                end
                                    ...
                                    if target ~= nil then
                                        vRP.prompt(
                                            ...
                                            function(player, amount)
                                                if
                                                    tonumber(amount) and tonumber(amount) > 0 and
                                                        tonumber(amount) < limit
                                                 then
                                                    MySQL.Sync_Query(
                                                        "vRP/get_vehicle",
                                                        ...
                                                        function(pvehicle, affected)
                                                            if #pvehicle > 0 then
                                                                vRPclient.notify(
                                                                    player,
                                                                    {"~r~The player already has this vehicle type."}
                                                                )
                                                            else
                                                                local tmpdata = vRP.getUserTmpTable(playerID)
                                                                MySQL.Sync_Query(
                                                                    "vRP/check_rented",
                                                                    {user_id = playerID, vehicle = veh},
                                                                    function(pvehicles)
                                                                        if #pvehicles > 0 then
                                                                            vRPclient.notify(
                                                                                player,
                                                                                {"~r~You cannot sell a rented vehicle!"}
                                                                            )
                                                                            return
                                                                        else
                                                                            vRP.request(
                                                                                ...
                                                                                function(target, ok)
                                                                                    if ok then
                                                                                        ...
                                                                                        if
                                                                                         ...
                                                                                         then
                                                                                            vRPclient.despawnGarageVehicle(
                                                                                                player,
                                                                                                {"car", 15}
                                                                                            )
                                                                                            vRP.getUserIdentity(
                                                                                            ...
                                                                                            vRPclient.notify(
                                                                                                player,
                                                                                                {
                                                                                                    "~g~You have successfully sold the vehicle to " ..
                                                                                                        GetPlayerName(
                                                                                                            target
                                                                                                        ) ..
                                                                                                            " for $" ..
                                                                                                ...
                                                                                                target,
                                                                                                {
                                                                                                    "~g~" ..
                                                                                                        GetPlayerName(
                                                                                                            ...
                                                                                                            " has successfully sold you the car for $" ..
                                                                                                                amount ..
                                                                                                                    "!"
                                                                                                }
                                                                                            )
                                                                                            ...
                                                                                            player,
                                                                                            {
                                                                                                "~r~" ..
                                                                                                    GetPlayerName(
                                                                                                        ...
                                                                                                        " has refused to buy the car."
                                                                                            }
                                                                                        )
                                                                                        vRPclient.notify(
                                                                                            target,
                                                                                            {
                                                                                                "~r~You have refused to buy " ..
                                                                                                    GetPlayerName(
                                                                                                        player
                                                                                                    ) ..
                                                                                                        "'s car."
                                                    ...
                                                    vRPclient.notify(
                                                        player,
                                                        {"~r~The price of the car has to be a number."}
                                                    )
                                                end
                                            end
                                        )
                                                    ...
                                                    function(player, rent)
                                                        if tonumber(rent) and tonumber(rent) > 0 then
                                                                                ...
                                                                                function(pvehicles)
                                                                                    if #pvehicles > 0 then
                                                                                        vRPclient.notify(
                                                                                            player,
                                                                                            {
                                                                                                "~r~You cannot rent a rented vehicle!"
                                                                                            }
                                                                                        )
                                                                                        return
                                                                                    else
                                                                                                    ...
                                                                                                    if
                                                                                                     ...
                                                                                                     then
                                                                                                        ...
                                                                                                        vRPclient.notify(
                                                                                                            player,
                                                                                                            {
                                                                                                                "~g~You have successfully rented the vehicle to " ..
                                                                                                                    GetPlayerName(
                                                                                                                        target
                                                                                                                    ) ..
                                                                                                                        " for $" ..
                                                                                                            ...
                                                                                                            target,
                                                                                                            {
                                                                                                                "~g~" ..
                                                                                                                    GetPlayerName(
                                                                                                                        ...
                                                                                                                        " has successfully rented you the car for $" ..
                                                                                                                            amount ..
                                                                                                                                "!" ..
                                                                                                                                    " | for: " ..
                                                                                                                                        rent ..
                                                                                                        ...
                                                                                                        player,
                                                                                                        {
                                                                                                            "~r~" ..
                                                                                                                GetPlayerName(
                                                                                                                    ...
                                                                                                                    " has refused to rent the car."
                                                                                                        }
                                                                                                    )
                                                                                                    vRPclient.notify(
                                                                                                        target,
                                                                                                        {
                                                                                                            "~r~You have refused to rent " ..
                                                                                                                GetPlayerName(
                                                                                                                    player
                                                                                                                ) ..
                                                                                                                    "'s car."
                                                            ...
                                                            vRPclient.notify(
                                                                player,
                                                                {
                                                                    "~r~The rent time of the car has to be in hours and a number."
                                                                }
                                                            )
                                                        end
                                                    end
...
veh_actions[lang.vehicle.trunk.title()] = {
    function(user_id, player, vtype, name)
        ...
        local max_weight =
            cfg_inventory.vehicle_chest_weights[string.lower(name)] or cfg_inventory.default_vehicle_chest_weight
                            ...
                            vRPclient.getNearestOwnedVehicle(
                                ...
                                function(ok, vtype, name)
                                    if ok then
                                        ...
                                        local max_weight =
                                            cfg_inventory.vehicle_chest_weights[string.lower(name)] or
                                        ...
                                        local cb_out = function(idname, amount)
                                            vRPclient.notify(
                                                nplayer,
                                                {lang.inventory.give.given({vRP.getItemName(idname), amount})}
                                            )
                                        end
                                        ...
                                        local cb_in = function(idname, amount)
                                            vRPclient.notify(
                                                nplayer,
                                                {lang.inventory.give.received({vRP.getItemName(idname), amount})}
                                            )
                                        end
...
AddEventHandler(
    ...
    function()
        ...
        if vRP.hasGroup(userid, "VIP") then
            ...
            vRPclient.notify(source, {"~r~You do not have VIP Garage Access!"})
        end
    end
)
