...
Citizen.CreateThread(function()
	MySQL.Create_Command("vRPls/update_vehicle_modifications", "update vrp_user_vehicles set modifications = @modifications where user_id = @user_id and vehicle = @vehicle")
	MySQL.Create_Command("vRPls/get_vehicle_modifications", "select modifications from vrp_user_vehicles where user_id = @user_id and vehicle = @vehicle")
    print("[vRP] init base tables")
end)

MySQL.Create_Command("vRP/create_user","INSERT INTO vrp_users(whitelisted,banned) VALUES(false,false)")
MySQL.Create_Command("vRP/add_identifier","INSERT INTO vrp_user_ids(identifier,user_id) VALUES(@identifier,@user_id)")
MySQL.Create_Command("vRP/userid_byidentifier","SELECT user_id FROM vrp_user_ids WHERE identifier = @identifier")
MySQL.Create_Command("vRP/identifier_all","SELECT * FROM vrp_user_ids WHERE identifier = @identifier")
MySQL.Create_Command("vRP/select_identifier_byid_all","SELECT * FROM vrp_user_ids WHERE user_id = @id")

MySQL.Create_Command("vRP/set_userdata","REPLACE INTO vrp_user_data(user_id,dkey,dvalue) VALUES(@user_id,@key,@value)")
MySQL.Create_Command("vRP/get_userdata","SELECT dvalue FROM vrp_user_data WHERE user_id = @user_id AND dkey = @key")

MySQL.Create_Command("vRP/set_srvdata","REPLACE INTO vrp_srv_data(dkey,dvalue) VALUES(@key,@value)")
MySQL.Create_Command("vRP/get_srvdata","SELECT dvalue FROM vrp_srv_data WHERE dkey = @key")

MySQL.Create_Command("vRP/get_banned","SELECT banned FROM vrp_users WHERE id = @user_id")
MySQL.Create_Command("vRP/set_banned","UPDATE vrp_users SET banned = @banned, bantime = @bantime,  banreason = @banreason,  banadmin = @banadmin WHERE id = @user_id")
MySQL.Create_Command("vRP/set_identifierbanned","UPDATE vrp_user_ids SET banned = @banned WHERE identifier = @iden")
MySQL.Create_Command("vRP/getbanreasontime", "SELECT * FROM vrp_users WHERE id = @user_id")

MySQL.Create_Command("vRP/get_whitelisted","SELECT whitelisted FROM vrp_users WHERE id = @user_id")
MySQL.Create_Command("vRP/set_whitelisted","UPDATE vrp_users SET whitelisted = @whitelisted WHERE id = @user_id")
MySQL.Create_Command("vRP/set_last_login","UPDATE vrp_users SET last_login = @last_login WHERE id = @user_id")
MySQL.Create_Command("vRP/get_last_login","SELECT last_login FROM vrp_users WHERE id = @user_id")

--Token Banning 
MySQL.Create_Command("vRP/add_token","INSERT INTO vrp_user_tokens(token,user_id) VALUES(@token,@user_id)")
MySQL.Create_Command("vRP/check_token","SELECT user_id, banned FROM vrp_user_tokens WHERE token = @token")
MySQL.Create_Command("vRP/check_token_userid","SELECT token FROM vrp_user_tokens WHERE user_id = @id")
MySQL.Create_Command("vRP/ban_token","UPDATE vrp_user_tokens SET banned = @banned WHERE token = @token")
--Token Banning
        ...
        local function search()
            ...
            if i <= #ids then
                if not config.ignore_ip_identifier or (string.find(ids[i], "ip:") == nil) then  -- ignore ip identifier
                    MySQL.Sync_Query("vRP/userid_byidentifier", {identifier = ids[i]}, function(rows, affected)
                ...
                MySQL.Sync_Query("vRP/create_user", {}, function(rows, affected)
                    if rows.affectedRows > 0 then
                        ...
                        for l,w in pairs(ids) do
                            if not config.ignore_ip_identifier or (string.find(w, "ip:") == nil) then  -- ignore ip identifier
                                MySQL.Execute_Command("vRP/add_identifier", {user_id = user_id, identifier = w})
                            end
                        end
            ...
            if vRP.rusers[user_id] == nil then -- not present on the server, init
                ...
                vRP.getUData(user_id, "vRP:datatable", function(sdata)
                    ...
                    vRP.getLastLogin(user_id, function(last_login)
                        ...
                        MySQL.Execute_Command("vRP/set_last_login", {user_id = user_id, last_login = last_login_stamp})
...
RegisterCommand("getmyid", function(source)
    TriggerClientEvent('chatMessage', source, "[Server]:", {255, 255, 255}, ("Your Perm ID is: %s"):format(vRP.getUserId(source)) , "alert")
end)
...
function vRP.isBanned(user_id, cbr)
    ...
    MySQL.Sync_Query("vRP/get_banned", {user_id = user_id}, function(rows, affected)
        if #rows > 0 then
            task({rows[1].banned})
        else
            task()
...
function vRP.isWhitelisted(user_id, cbr)
    ...
    MySQL.Sync_Query("vRP/get_whitelisted", {user_id = user_id}, function(rows, affected)
        if #rows > 0 then
...
function vRP.setWhitelisted(user_id,whitelisted)
    MySQL.Execute_Command("vRP/set_whitelisted", {user_id = user_id, whitelisted = whitelisted})
end

--- sql
function vRP.getLastLogin(user_id, cbr)
    local task = Task(cbr,{""})
    MySQL.Sync_Query("vRP/get_last_login", {user_id = user_id}, function(rows, affected)
        if #rows > 0 then
...
function vRP.fetchBanReasonTime(user_id,cbr)
    MySQL.Sync_Query("vRP/getbanreasontime", {user_id = user_id}, function(rows, affected)
        if #rows > 0 then 
...
function vRP.setUData(user_id,key,value)
    MySQL.Execute_Command("vRP/set_userdata", {user_id = user_id, key = key, value = value})
end

function vRP.getUData(user_id,key,cbr)
    local task = Task(cbr,{""})
    
    MySQL.Sync_Query("vRP/get_userdata", {user_id = user_id, key = key}, function(rows, affected)
        if #rows > 0 then
            task({rows[1].dvalue})
        else
            task()
...
function vRP.setSData(key,value)
    MySQL.Execute_Command("vRP/set_srvdata", {key = key, value = value})
end

function vRP.getSData(key, cbr)
    local task = Task(cbr,{""})
    
    MySQL.Sync_Query("vRP/get_srvdata", {key = key}, function(rows, affected)
...
function vRP.IdentifierBanCheck(source,user_id,cb)
    for i,v in pairs(GetPlayerIdentifiers(source)) do 
        MySQL.Sync_Query('vRP/identifier_all', {identifier = v}, function(rows)
            for i = 1,#rows do 
...
function vRP.BanIdentifiers(user_id, value)
    MySQL.Sync_Query('vRP/select_identifier_byid_all', {id = user_id}, function(rows)
        for i = 1, #rows do 
            MySQL.Execute_Command("vRP/set_identifierbanned", {banned = value, iden = rows[i].identifier })
        end
    end)
end
...
function vRP.setBanned(user_id,banned,time,reason, admin)
    if banned then
        ...
        MySQL.Execute_Command("vRP/set_banned", {user_id = user_id, banned = banned, bantime = time, banreason = reason, banadmin = admin})
        ...
        MySQL.Execute_Command("vRP/set_banned", {user_id = user_id, banned = banned, bantime = "", banreason =  "", banadmin =  ""})
...
function vRP.StoreTokens(source, user_id) 
    if GetNumPlayerTokens then 
        ...
        for i = 1, numtokens do
            ...
            MySQL.Sync_Query("vRP/check_token", {token = token}, function(rows)
                if token and rows and #rows <= 0 then 
                    MySQL.Execute_Command("vRP/add_token", {token = token, user_id = user_id})
                end        
            end)
        end
    end
...
function vRP.CheckTokens(source, user_id) 
    if GetNumPlayerTokens then 
        ...
        for i = 1, numtokens do
            local token = GetPlayerToken(source, i)
            local rows = MySQL.Async_Query("vRP/check_token", {token = token, user_id = user_id})
                if #rows > 0 then 
...
function vRP.BanTokens(user_id, banned) 
    if GetNumPlayerTokens then 
        MySQL.Sync_Query("vRP/check_token_userid", {id = user_id}, function(id)
            for i = 1, #id do 
                MySQL.Execute_Command("vRP/ban_token", {token = id[i].token, banned = banned})
            end
        end)
    end
end
...
AddEventHandler("vRPcli:playerSpawned", function()
    ...
    if user_id ~= nil then
        ...
        if first_spawn then
            ...
            vRPclient.setAdmin(source, {vRP.hasPermission(user_id, "admin.tickets")})
            vRPclient.setFounder(source, {vRP.hasGroup(user_id, 'founder')})
        end
