...
function tvRP.varyHealth(variation)
    ...
    if variation == 200 then
        return TriggerEvent("Nova:FixClient", ped)
    end
...
function tvRP.setFriendlyFire(flag)
    ...
    SetCanAttackFriendly(PlayerPedId(), flag, false)
end
...
Citizen.CreateThread(
    function()
        ...
        while true do
            ...
            if IsEntityDead(ped) and not in_coma then --Wait for death check
                pbCounter = 100
                local plyCoords = GetEntityCoords(ped, true)
        ...
        while true do
            ...
            if IsEntityDead(PlayerPedId()) then
                ...
                if DeathReason == "committed suicide" or DeathReason == "died" then
                    ...
                    DeathString =
                        string.upper(
                        "You were " .. DeathReason .. " by " .. GetPlayerName(Killer) .. " with a " .. tostring(Weapon)
                    )
                end
                Killer = nil
                DeathReason = nil
