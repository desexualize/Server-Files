...
RMenu.Add("vRPATM", "main", RageUI.CreateMenu("", "~g~Nova ATM Menu", 1300, 50, "atm", "atm"))
RageUI.CreateWhile(
    1.0,
    true,
    function()
        if RageUI.Visible(RMenu:Get('vRPATM', 'main')) then
            RageUI.DrawContent({ header = true, glare = false, instructionalButton = true}, function()
                    ...
                    if Selected then 
                        ...
                        if (GetOnscreenKeyboardResult()) then
                            ...
                            if result then 
                                ...
                                TriggerServerEvent('Nova:Deposit', result)
                ...
                RageUI.Button("Withdraw", nil, {}, true, function(Hovered, Active, Selected) 
                    if Selected then 
                        ...
                        if (GetOnscreenKeyboardResult()) then
                            ...
                            if result then 
                                result = tonumber(result)
                                TriggerServerEvent('Nova:Withdraw', result)
                                PlaySoundFrontend(-1, "Bomb_Disarmed", "GTAO_Speed_Convoy_Soundset", 0)
                            end
                        end
                    end
                ...
                RageUI.Button("Deposit All", nil, {}, true, function(Hovered, Active, Selected) 
                    if Selected then 
                        TriggerServerEvent("Nova:DepositAll")
                        PlaySoundFrontend(-1, "Bomb_Disarmed", "GTAO_Speed_Convoy_Soundset", 0)
                    end
                end)
                RageUI.Button("Withdraw All", nil, {}, true, function(Hovered, Active, Selected) 
                    if Selected then 
                        TriggerServerEvent("Nova:WithdrawAll")
                        PlaySoundFrontend(-1, "Bomb_Disarmed", "GTAO_Speed_Convoy_Soundset", 0)
                    end
                end)
            end)
