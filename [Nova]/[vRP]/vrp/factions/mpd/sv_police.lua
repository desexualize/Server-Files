...
--                             vRP.sendServiceAlert(
--                               nil,
--                               cfg.trackveh.service,
--                               x,
--                               y,
--                               z,
--                               lang.police.pc.trackveh.tracked({reg, note})
--                             )
--                           else
...
AddEventHandler("Nova:AskID", function()
    --choice_askid()
    local player = source
    vRPclient.getNearestPlayer(player, {10}, function(nplayer)
		local nuser_id = vRP.getUserId(nplayer)
							...
							if amount > 0 then
								...
								if item then -- do transfer
									if vRP.tryGetInventoryItem(nuser_id, v, amount, true) then
										...
										vRPclient.notify(player, {"~b~Seized Players Weapons"})
										vRPclient.notify(nplayer, {"~b~Your Items Have Been Seized"})
									end
								end
							end
						end
...
local choice_store_weapons = function(player, choice)
	...
	vRPclient.getWeapons(player, {}, function(weapons)
		if not isStoring[player] then
			...
			if vRP.getInventoryWeight(user_id) <= 25 then
				...
				vRPclient.notify(player, {"~r~You do not have enough Weight to store Weapons."})
			end
		end
	end)
end
