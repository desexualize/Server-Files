local lang = vRP.lang
local cfg = module('vrp', "factions/mpd/cfg_mpd")

RegisterServerEvent("Nova:OpenPoliceMenu")
AddEventHandler("Nova:OpenPoliceMenu", function()
    local source = source
    local user_id = vRP.getUserId(source)
    if user_id ~= nil and vRP.hasPermission(user_id, "police.menu") then
      	TriggerClientEvent("Nova:PoliceMenuOpened", source)

    elseif user_id ~= nil and vRP.hasPermission(user_id, "mpd_clockon.menu") then
          ...
          if nplayer ~= nil then
            ...
            if nuser_id ~= nil then
              vRPclient.isHandcuffed(
                ...
                function(handcuffed)
                  if handcuffed then
                    TriggerClientEvent("Nova:DragPlayer", nplayer, player)
                  else
...
RegisterServerEvent("Nova:Handcuff")
AddEventHandler(
  "Nova:Handcuff",
  function()
    player = source
...
RegisterServerEvent("Nova:JailPlayer")
AddEventHandler(
  "Nova:JailPlayer",
  function()
    player = source
...
RegisterServerEvent("Nova:UnJailPlayer")
AddEventHandler(
  "Nova:UnJailPlayer",
  function()
    player = source
                              ...
                              if target ~= nil then
                                ...
                                if vRP.tryFullPayment(tonumber(target_id), tonumber(fine)) then
                                  vRP.insertPoliceRecord(
                                    tonumber(target_id),
                                    lang.police.menu.fine.record({reason, fine})
                                  )
...
RegisterNetEvent("Nova:PutPlrInVeh")
AddEventHandler(
  "Nova:PutPlrInVeh",
  function()
    player = source
...
RegisterNetEvent("Nova:TakeOutOfVehicle")
AddEventHandler(
  "Nova:TakeOutOfVehicle",
  function()
    player = source
    local user_id = vRP.getUserId(player)
    if user_id ~= nil and vRP.hasPermission(user_id, "police.menu") then
...
RegisterNetEvent("Nova:SearchPlayer")
AddEventHandler(
  "Nova:SearchPlayer",
  function()
    player = source
