...
AddEventHandler(
	...
	function(pumpObject, ped, vehicle)
		...
		while isFueling do
			...
			if pumpObject then
				...
				DrawText3Ds(
					...
					Config.Strings.CancelFuelingJerryCan ..
						"\nGas can: ~g~" ..
							Round(GetAmmoInPedWeapon(ped, 883325847) / 4500 * 100, 1) .. "% | Vehicle: " .. Round(currentFuel, 1) .. "%"
				)
			end
			...
			if
				IsControlJustReleased(0, 38) or DoesEntityExist(GetPedInVehicleSeat(vehicle, -1)) or
					(isNearPump and GetEntityHealth(pumpObject) <= 0)
			 then
				isFueling = false
			end
...
Citizen.CreateThread(
	function()
		while true do
			...
			if
				not isFueling and
					((isNearPump and GetEntityHealth(isNearPump) > 0) or (GetSelectedPedWeapon(ped) == 883325847 and not isNearPump))
			 then
									...
									if refillCost > 0 then
										if currentCash >= refillCost then
											DrawText3Ds(
												...
												stringCoords.y,
												stringCoords.z + 1.2,
												Config.Strings.RefillJerryCan .. refillCost
											)
