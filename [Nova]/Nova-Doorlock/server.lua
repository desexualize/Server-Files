...
local cfg = module("Nova-Doorlock", "config")
  ...
  if vRP.hasPermission({user_id, "#"..cfg.list[id].key..".>0"}) or vRP.hasPermission({user_id,cfg.list[id].permission}) then
    ...
    SetTimeout(4000, function()
      ...
      if cfg.list[id].locked then
        vRPclient.notify(player, {"Door Locked "..cfg.list[id].name})
      else
        vRPclient.notify(player, {"Door Unlocked "..cfg.list[id].name})
      end
    end)
  else
    vRPclient.notify(player, {"You do not have perms to open this door! "..cfg.list[id].name})
  end
end)
