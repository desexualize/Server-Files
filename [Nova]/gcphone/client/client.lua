...
function ShowNoPhoneWarning ()
  ...
  AddTextComponentSubstringPlayerName('~o~No Phone')
...
function showFixePhoneHelper (coords)
  for number, data in pairs(FixePhone) do
    ...
    if dist <= 2.0 then
      ...
      AddTextComponentSubstringPlayerName("~g~" .. data.name .. ' ~o~' .. number .. '~n~~INPUT_PICKUP~~w~ Use')
  ...
  while true do 
    ...
    for i, _ in pairs(PhoneInCall) do 
        ...
        if (dist <= soundDistanceMax) then
          ...
          if (dist <= 1.5) then 
            SetTextComponentFormat("STRING")
            AddTextComponentSubstringPlayerName("~INPUT_PICKUP~ Detach")
