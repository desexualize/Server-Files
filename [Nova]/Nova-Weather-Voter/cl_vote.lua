...
function voteWeather(weatherType) 
	...
	if voteInProgress then
		if not voted then
			TriggerServerEvent("Nova:vote", weatherType)
			voted = true
...
function changeWeather()
	...
	TriggerServerEvent("Nova:setCurrentWeather",highestWeather)
end

RegisterNetEvent("Nova:voteFinished")
AddEventHandler("Nova:voteFinished", function(newWeather)
	currentWeather = newWeather
end)

RegisterNetEvent("Nova:startWeatherVote")
AddEventHandler("Nova:startWeatherVote", function()
	voteInProgress = true
...
RegisterNetEvent("Nova:voteStateChange")
AddEventHandler("Nova:voteStateChange",function(type)
	votesTable[type] = votesTable[type] + 1
end)

RegisterCommand("voteweather", function()
	TriggerServerEvent("Nova:tryStartWeatherVote")
end, false)
...
AddEventHandler('playerSpawned', function(spawn)
    TriggerServerEvent("Nova:getCurrentWeather")
