            ...
            if EntityType then
                ...
                if playerIsAlive() and playerVehicle == 0 then
                    if EntityType == 1 and Entity ~= playerPed and IsPedAPlayer(Entity) then
                        Crosshair(true)
                        if
                            IsDisabledControlPressed(0, keybindControls[keyBind]) and GetLastInputMethod(2) and
                                not showMenu
                         then
                            Crosshair(false)
                            TriggerServerEvent("Nova:PoliceCheck")
                            TriggerServerEvent("Nova:NHSCheck")
                            showMenu = true
                            local enabledMenus = {}
                            for _, menuConfig in ipairs(rootMenuConfig) do
                                if menuConfig:enableMenu() then
                                    ...
                                    if menuConfig.subMenus ~= nil and #menuConfig.subMenus > 0 then
                                        ...
                                        TriggerEvent("NovaCli:Notify", txt)
                                        hasSubMenus = true
                                        local previousMenu = dataElements
                                        local currentElement = {}
                                        for i = 1, #menuConfig.subMenus do
                                            if
                                                newSubMenus[menuConfig.subMenus[i]] ~= nil and
                                                    newSubMenus[menuConfig.subMenus[i]].enableMenu ~= nil and
                                                    not newSubMenus[menuConfig.subMenus[i]]:enableMenu()
                                             then
                                            else
                                                currentElement[#currentElement + 1] =
                                                    newSubMenus[menuConfig.subMenus[i]]
                            ...
                            Wait(100)
                            while IsDisabledControlPressed(0, keybindControls[keyBind]) and GetLastInputMethod(2) do
                                Wait(100)
                            end
                        end
                    else
                ...
                elseif IsPedInAnyVehicle(PlayerPedId()) then
                    if 1 == 1 then
                        Crosshair(true)
                        if
                            IsDisabledControlPressed(0, keybindControls[inCarBind]) and GetLastInputMethod(2) and
                                not showMenu
                         then
                            Crosshair(false)
                            TriggerServerEvent("Nova:PoliceCheck")
