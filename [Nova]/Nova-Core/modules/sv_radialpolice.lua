...
RegisterNetEvent("Nova:policeDrag")
AddEventHandler(
  "Nova:policeDrag",
  function()
    local source = source
          ...
          if nplayer ~= nil then
            ...
            if nuser_id ~= nil then
              vRPclient.isHandcuffed(
                ...
                function(handcuffed)
                  if handcuffed then
                    TriggerClientEvent("Nova:drag2", nplayer, source)
                  else
...
RegisterNetEvent("Nova:policeCuff")
AddEventHandler(
  "Nova:policeCuff",
  function()
    local player = source
            ...
            function(handcuffed)
              ...
              if nuser_id ~= nil then
                if handcuffed == false then
                  ...
                  TriggerClientEvent("Nova:cuffAnimPolice", player, nplayer, player)
                  TriggerClientEvent("Nova:cuffAnimPerp", nplayer, nplayer, player)
...
RegisterServerEvent("Nova:toggleTrafficMenu")
AddEventHandler(
  "Nova:toggleTrafficMenu",
  function()
    local source = source
    local user_id = vRP.getUserId({source})
    if user_id ~= nil and vRP.hasPermission({user_id, "police.menu"}) then
      print("vRP Identification Accepted")
      TriggerClientEvent("Nova:openTrafficMenu", source)
    end
  end
)

RegisterServerEvent("Nova:zoneActivated")
AddEventHandler(
  "Nova:zoneActivated",
  function(message, speed, radius, x, y, z)
    TriggerClientEvent("chatMessage", -1, message)
    TriggerClientEvent("Nova:createZone", -1, speed, radius, x, y, z)
  end
)

RegisterServerEvent("Nova:disableZone")
AddEventHandler(
  "Nova:disableZone",
  function(blip)
    TriggerClientEvent("Nova:removeBlip", -1)
  end
)
                                  ...
                                  if reason ~= nil and reason ~= "" then
                                    ...
                                    if target ~= nil then
                                      ...
                                      if vRP.tryFullPayment({tonumber(target_id), tonumber(fine)}) then
                                        vRP.insertPoliceRecord(
                                          {tonumber(target_id), lang.police.menu.fine.record({reason, fine})}
                                        )
                      ...
                      if custom ~= nil then
                        ...
                        if tonumber(custom) > 0 or vRP.hasPermission({user_id, "admin.easy_unjail"}) then
                          ...
                          if target ~= nil then
                            ...
                            vRPbm.logInfoToFile(
                              "jailLog.txt",
                              user_id .. " freed " .. target_id .. " from a " .. custom .. " minutes sentence"
                            )
                          else
                            vRPclient.notify(player, {"~r~That ID seems invalid."})
                          end
...
RegisterNetEvent("Nova:putinveh")
AddEventHandler(
  "Nova:putinveh",
  function()
    player = source
        ...
        function(nplayer)
          ...
          if nuser_id ~= nil then
            vRPclient.isHandcuffed(
              nplayer,
              {},
              function(handcuffed) -- check handcuffed
                if handcuffed then
...
RegisterNetEvent("Nova:takeoutveh")
AddEventHandler(
  "Nova:takeoutveh",
  function()
    player = source
...
RegisterServerEvent("Nova:SeizeWeapons")
AddEventHandler(
  "Nova:SeizeWeapons",
  function()
    local player = source
...
--[[
RegisterServerEvent('Nova:SeizeItems')
AddEventHandler('Nova:SeizeItems', function()
  local player = source 
  local user_id = vRP.getUserId({player})
  if user_id ~= nil then
...
RegisterServerEvent("Nova:TrafficMenuLogs")
AddEventHandler(
  "Nova:TrafficMenuLogs",
  function(objspawned)
    local player = source
    local admin_id = vRP.getUserId({player})
    local admin_name = GetPlayerName(player)
    local logs = ""
    local communityname = "Nova RP Staff Logs"
    ...
    local command = {
      {
        ...
        ["description"] = "** Admin Name: **" ..
          admin_name .. "** Admin ID:  **" .. admin_id .. "** Spawned Objects : **" .. objspawned .. "",
        ["footer"] = {
          ["text"] = communityname,
    ...
    PerformHttpRequest(
      ...
      "POST",
      json.encode({username = "Nova RP Staff Logs", embeds = command}),
