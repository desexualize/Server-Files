...
RegisterNetEvent("Nova:isPoliceVar")
AddEventHandler(
	"Nova:isPoliceVar",
	function(target)
	end
...
RegisterNetEvent("Nova:getTackled")
AddEventHandler(
	"Nova:getTackled",
	function(target)
...
RegisterNetEvent("Nova:playTackle")
AddEventHandler(
	"Nova:playTackle",
	function()
		isTackling = true
	...
	function()
		while true do
			...
			if IsControlPressed(0, Keys["LEFTSHIFT"], true) and IsControlPressed(0, Keys["G"], true) and not isTackling then
				...
				if
					distance ~= -1 and distance <= 3.0 and not isTackling and not isGettingTackled and
						not IsPedInAnyVehicle(PlayerPedId()) and
						not IsPedInAnyVehicle(GetPlayerPed(closestPlayer))
				 then
					lastTackleTime = GetGameTimer()
					TriggerServerEvent("Nova:tryTackle", closestPlayerId)
				end
			end
		end
	end
