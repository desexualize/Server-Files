TriggerServerEvent("Nova:Joining", source)
...
Citizen.CreateThread(
	function()
		...
		while config == nil or config.initialized == nil or cor == nil or cor.initialized == nil or stealth == nil or
			stealth.initialized == nil do
			Citizen.Wait(0)
		end
		...
		while true do
			...
			if toggleCrouchWithDuckBind and duckBindPressed() and not noclipActive then
				if
					IsPedOnFoot(playerped) and not IsPedJumping(playerped) and not IsPedFalling(playerped) and not IsPlayerDead(player)
				 then
