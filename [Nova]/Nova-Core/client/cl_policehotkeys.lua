...
RegisterNetEvent("Nova:policeHotkeysTrue")
AddEventHandler(
    "Nova:policeHotkeysTrue",
    function()
        handcuffed = true
    end
)

RegisterNetEvent("Nova:policeHotkeysFalse")
AddEventHandler(
    "Nova:policeHotkeysFalse",
    function()
        handcuffed = false
    end
)
    ...
    function()
        while true do
            ...
            if IsControlPressed(0, 19, true) and IsControlPressed(0, 30, true) then
                TriggerServerEvent("Nova:policeDrag")
    ...
    function()
        while true do
            ...
            if IsControlPressed(0, 344, true) then
                TriggerServerEvent("Nova:policeCuff")
                Wait(1000)
            end
        end
    end
...
RegisterNetEvent("Nova:cuffAnimPolice")
AddEventHandler(
    "Nova:cuffAnimPolice",
    function(nplayer, player)
        local player = player
        local nplayer = nplayer
        TriggerServerEvent("Nova:PlayWithinDistance", 0.2, "cuffing", 1, GetEntityCoords(PlayerPedId()))
...
RegisterNetEvent("Nova:cuffAnimPerp")
AddEventHandler(
    "Nova:cuffAnimPerp",
    function()
        TriggerServerEvent("Nova:PlayWithinDistance", 0.2, "cuffing", 1, GetEntityCoords(PlayerPedId()))
