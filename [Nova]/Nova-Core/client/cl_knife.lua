RMenu.Add("KnifeStoreMenu", "main", RageUI.CreateMenu("", "~r~Nova Kinife Store", 1300, 50, "knife", "knife"))
RMenu.Add("KnifeStoreMenu", "confirm", RageUI.CreateSubMenu(RMenu:Get("KnifeStoreMenu", "main", 1300, 50)))

RageUI.CreateWhile(
    1.0,
    RMenu:Get("KnifeStoreMenu", "main"),
    nil,
    function()
        RageUI.IsVisible(
            RMenu:Get("KnifeStoreMenu", "main"),
            ...
            function()
                for i, p in pairs(knife.knives) do
                    RageUI.Button(
                        p.name,
                        nil,
                        {RightLabel = knife.currency .. tostring(p.price)},
                        true,
                        ...
                        RMenu:Get("KnifeStoreMenu", "confirm")
                    )
                end
            end,
            function()
        ...
        RageUI.IsVisible(
            ...
            function()
                ...
                RageUI.Button(
                    ...
                    function(Hovered, Active, Selected)
                        if Selected then
                            TriggerServerEvent("Nova:BuyKnife", cPrice, cHash)
                        end
                    end,
                    RMenu:Get("KnifeStoreMenu", "main")
                )
...
RegisterNetEvent("Nova:GiveKnife")
AddEventHandler(
    "Nova:GiveKnife",
    function(hash)
        -- AntiCheat Allow Weapon Trigger
        GiveWeaponToPed(PlayerPedId(), hash, 150, false, false, 0)
    end
)

RegisterNetEvent("Nova:SetArmour")
AddEventHandler(
    "Nova:SetArmour",
    function(level)
...
Citizen.CreateThread(
    function(v1)
        ...
        AddTextComponentSubstringPlayerName("Knife Shop")
