...
RMenu.Add("TruckJob", "main", RageUI.CreateMenu("Truck Job", "Nova Truck Job", 1300, 50))

RageUI.CreateWhile(
    1.0,
    RMenu:Get("TruckJob", "main"),
    nil,
    function()
        RageUI.IsVisible(
            RMenu:Get("TruckJob", "main"),
...
function tick()
    ...
    elseif (MISSION.start == true) then
        ...
        if
            (GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords) < 25 and
                not IsEntityAttached(MISSION.trailer))
         then
        ...
        elseif
            (GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords) < 25 and
                IsEntityAttached(MISSION.trailer))
         then
...
function MISSION.spawnTruck()
    MISSION.truck =
        CreateVehicle(MISSION.hashTruck, 930.04858398438, -3119.5952148438, 5.9008030891418, 0.0, true, false)
...
function MISSION.removeMarker()
    SetBlipSprite(BLIP.destination[BLIP.destination.i], 2)
     --invisible
    SetBlipSprite(BLIP.trailer[BLIP.trailer.i], 2) --invisible
end
...
function GUI.renderTitle()
    for id, settings in pairs(GUI.title) do
        ...
        EndTextCommandDisplayText((settings["xpos"] + 0.001), (settings["ypos"] - 0.015))
        --AddTextComponentSubstringPlayerName(settings["name"])
        GUI.renderBox(
            settings["xpos"],
            settings["ypos"],
...
function GUI.renderDesc()
    for id, settings in pairs(GUI.desc) do
        ...
        EndTextCommandDisplayText((settings["xpos"] - 0.06), (settings["ypos"] - 0.13))
        AddTextComponentSubstringPlayerName(settings["name"])
        GUI.renderBox(
            settings["xpos"],
            settings["ypos"],
