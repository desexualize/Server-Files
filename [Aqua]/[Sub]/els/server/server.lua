...
local resourceName = "els" .. (GetCurrentResourceName() ~= "els" and " (" .. GetCurrentResourceName() .. ")" or "")
local latestVersionPath = "https://raw.githubusercontent.com/MrDaGree/els/master/els/version.json"
local curVerCol = function(nextVer)
    return
        (curVersion < nextVer) and "~d~" or
...
function checkVersion()
	PerformHttpRequest(latestVersionPath, function(err, response, headers)
		...
		if curVersion == 0 then
			return print("^1Could not determine which version of els you are running.")
		end
		...
		if latestVersion:sub(-1) == "b" then
			...
			print("Please report any regressions you find on the GitHub page (^4https://github.com/MrDaGree/els^1")
			print("--------------------------------------------------------------------------^7")
		elseif curVersion ~= data_version and curVersion < lVer then
			print("--------------------------------------------------------------------------")
			print(resourceName .. " is outdated.\nCurrent Version: " .. data_version .. "\nYour Version: " .. curVersion .. "\nPlease update it from https://github.com/MrDaGree/els")
			print("\nUpdate Changelog:\n"..data.changelog)
			print("\n--------------------------------------------------------------------------")
...
RegisterCommand('els', function(source, args)
    ...
    if args[1] == 'version' then
        PerformHttpRequest(latestVersionPath, function(err, response, headers)
            ...
            if source > 0 then
                return notify(source, "~d~ELS~s~~n~Version " .. curVerCol(thisVersion) .. curVersion)
            end
            ...
            elseif curVersion > thisVersion then
                message = "Um, what? Your version of els is higher than the current version. What?"
            else
                message = "Your version of [ " .. GetCurrentResourceName() .. " ] is up to date! Current version: [ " .. curVersion .. " ]."
            end

            if message then
                return print("els (" .. GetCurrentResourceName() .. "): " .. message)
            end

            return print("els (" .. GetCurrentResourceName() .. "): Could not find any relationship between the latest version and the version you are running.")
        end, "GET", "", {version = 'this'})
    elseif args[1] == 'panel' then
        if source == 0 then return end
        if not args[2] then
            TriggerClientEvent("chat:addMessage", source, {
                args = { "els", "Please input a panel type! " },
                color = {13, 161, 200}
            })
            return
        end
    ...
    elseif not args[1] or args[1] == 'help' then
        TriggerClientEvent("els:notify", source, "~d~ELS~s~~n~Version " .. curVerCol(latestVersion) .. curVersion)
        TriggerClientEvent("els:notify", source, "~d~Sub-Commands~s~~n~" .. "~p~panel~s~ - Sets the panel type, options: " .. table.concat(allowedPanelTypes, ", ") .. "~n~~p~version~s~ - Shows current version and if the owner should update or not.~n~~p~help~s~ - Displays this notification.")
    end
end)
...
AddEventHandler("els:playerSpawned", function()
    ...
    if curVersion < latestVersion then
        TriggerClientEvent("els:notify", source, "~d~els~s~~n~Outdated version! Please update as soon as possible.")
    elseif curVersion > latestVersion then
        TriggerClientEvent("els:notify", source, "~o~els~s~~n~The current version is higher than latest! Please downgrade or check for updates.")
    else
        return
    end
end)
...
AddEventHandler('onResourceStart', function(name)
    if name:lower() == GetCurrentResourceName():lower() then
        ...
        assert(vcfType == "table", string.format("Expected type 'table' for vcf_files, got %s. %s", vcfType, vcfType == "nil" and 
            "Please make sure you have a file called 'vcf.lua' in the resource root folder (resources/els or similar). Copy the contents of 'vcf.default.lua' " ..
