...
-- Script starts here
local helicam = false
local polmav_hash = GetHashKey("npas")
			...
			if IsHeliHighEnough(heli) then
				...
				if IsControlJustPressed(0, toggle_rappel) then -- Initiate rappel
					...
					if GetPedInVehicleSeat(heli, 1) == lPed or GetPedInVehicleSeat(heli, 2) == lPed then
						...
						AddTextComponentString("~d~Can't rappel from this seat")
...
function IsPlayerInPolmav()
	...
	if IsVehicleModel(vehicle, GetHashKey("polisheli")) then
		return true
	elseif IsVehicleModel(vehicle, GetHashKey("littlebird")) then
		return true
	elseif IsVehicleModel(vehicle, GetHashKey("npas")) then
		return true
	end
end
