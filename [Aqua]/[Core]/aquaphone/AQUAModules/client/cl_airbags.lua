...
Citizen.CreateThread(
    function()
        while true do
            local e = tAQUA.getPlayerVehicle()
            if e ~= 0 and GetPedInVehicleSeat(e, -1) == tAQUA.getPlayerPed() then
                ...
                if f <= b and a[e] == nil then
                    ...
                    if a[e] == nil then
                        d = GetPedArmour(tAQUA.getPlayerPed())
                    end
                end
            else
                d = 0
...
function createAirbags(e)
    ...
    if d > GetPedArmour(tAQUA.getPlayerPed()) then
        tAQUA.setArmour(d)
    end
    d = 0
    SetTimeout(
