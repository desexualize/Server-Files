...
RMenu.Add('AQUAblackjack', 'instructions', RageUI.CreateMenu("", "test",GetRageUIMenuWidth(),GetRageUIMenuHeight(),"casinoui_cards_blackjack", "casinoui_cards_blackjack"))
RMenu:Get('AQUAblackjack', 'instructions'):SetSubtitle("~d~BLACKJACK")
RMenu.Add('AQUAblackjack_high', 'instructions', RageUI.CreateMenu("", "test",GetRageUIMenuWidth(),GetRageUIMenuHeight(),"casinoui_cards_blackjack_high", "casinoui_cards_blackjack_high"))
RMenu:Get('AQUAblackjack_high', 'instructions'):SetSubtitle("~d~BLACKJACK")


RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('AQUAblackjack', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            ...
            end, RMenu:Get('AQUAblackjack', 'instructions'))            
        end, function()
            ---Panels
        end)
    end
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('AQUAblackjack_high', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            RageUI.FakeButton("test", "The aim of Blackjack is to beat the Dealer's hand without going over 21. This game uses four 52           card decks, which are shuffled at the start of every hand.                                                                                    The dealer will stand on soft 17.", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
            ...
            end, RMenu:Get('AQUAblackjack_high', 'instructions'))            
        end, function()
            ---Panels
        end)
    end
...
function showHowToBlackjack(flag)
    if closestChair < 8 then 
        RageUI.Visible(RMenu:Get('AQUAblackjack', 'instructions'), flag)
    else 
        RageUI.Visible(RMenu:Get('AQUAblackjack_high', 'instructions'), flag)
    end
end
...
AddEventHandler("Blackjack:blackjackLose",function(tableId)
    ...
    angryILost()
    canExitBlackjack = true
    drawNativeNotification("~d~You lose!")
    drawCurrentHand = false
    currentHand = 0
    dealersHand = 0
end)
...
AddEventHandler("Blackjack:blackjackPush",function(tableId)
    ...
    annoyedIPushed()
    canExitBlackjack = true
    drawNativeNotification("~d~You pushed!")
    drawCurrentHand = false
    currentHand = 0
    dealersHand = 0
end)
