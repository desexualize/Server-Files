...
RMenu.Add("AQUA:Identity", "main", RageUI.CreateMenu("Identity Shop", "~d~Identity Shop", GetRageUIMenuWidth(),GetRageUIMenuHeight()))

RageUI.CreateWhile(1.0, RMenu:Get("AQUA:Identity", "main"), nil, function()
    RageUI.IsVisible(RMenu:Get("AQUA:Identity", "main"), true, false, true, function()
                            ...
                            if secondName ~= "" then  
                                ...
                                if (GetOnscreenKeyboardResult()) then
                                    ...
                                    if tonumber(age) and tonumber(age) < 100 then 
                                        TriggerServerEvent("AQUA:ChangeIdentity", firstName, secondName, tonumber(age))
                                    else
                                        notify("~d~Invalid age!")
                                    end
                                end
                            else
                                notify("~d~Invalid second name!")
                            end
                        end
                    else
                        notify("~d~Invalid first name!")
                    end
                end
            end
        end)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if isInArea(cfg.location, 1.0) and inMenu == false then
            ...
            RageUI.Visible(RMenu:Get("AQUA:Identity", "main"), true)
            inMenu = true
            end
        end
        ...
        if isInArea(cfg.location, 1.0) == false and inMenu then
            inMenu = false
            RageUI.Visible(RMenu:Get("AQUA:Identity", "main"), false)
        end
    end
end)
