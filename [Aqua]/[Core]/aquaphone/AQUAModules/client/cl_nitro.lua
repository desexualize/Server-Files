...
RegisterCommand("nitro", function()
    if NitroCooldown == 0 then
        if IsPedInAnyVehicle(PlayerPedId(), true) then
            ShowNotification("~d~You can't be in a vehicle!")
        else
            TriggerServerEvent("CheckNitro")
        end
    else
        ShowNotification("~d~Please wait a few seconds before using the command again!")
    end
end)

RegisterNetEvent("NotBoosing")
AddEventHandler("NotBoosing", function()
    ShowNotification("~d~You are not a nitro booster.")
end)
