...
TriggerEvent('AQUA:RobberyBlip', bank, false)
...
AddEventHandler('es_bank:currentlyrobbing', function(robb)
	...
	--TriggerEvent('AQUA:RobberyBlip', bank, true)
end)
...
Citizen.CreateThread(function()
	...
	while true do
				...
				if not robbing then
					...
					if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 2)then
						if (incircle == false) then
							bank_DisplayHelpText("Press ~INPUT_CONTEXT~ to rob ~d~" .. v.nameofbank .. "~w~ beware, the police will be alerted!")
						end
						incircle = true
						if(IsControlJustReleased(1, 51))then
							if IsPedArmed(PlayerPedId(), 4) then
								...
								notify("~d~You need to be armed to rob the store!")
							end
						end
		...
		if robbing then
			...
			bank_drawTxt(0.92, 1.44, 1.0,1.0,0.5, "Robbing store: ~d~" .. secondsRemaining .. "~w~ seconds remaining", 255, 255, 255, 255)
