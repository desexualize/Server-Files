...
function callTakeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("~d~You need a pistol out with ammo to take a hostage at gunpoint!")
	end
	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				drawNativeNotification("~d~No one nearby to take as hostage!")
			end
		else
			drawNativeNotification("~d~No one nearby to take as hostage!")
		end
...
Citizen.CreateThread(function()
	...
	while true do 
		if takeHostage.type == "agressor" then
			...
			drawNativeText("~d~Press [G] to release, [H] to kill")
