...
RMenu.Add("AQUA:Shops", "main", RageUI.CreateMenu("Shop", "~d~Shop", GetRageUIMenuWidth(),GetRageUIMenuHeight()))
RMenu.Add("AQUA:Shops", "sub", RageUI.CreateSubMenu(RMenu:Get("AQUA:Shops", "main"), "Shop", "~d~Confirm Purchase", GetRageUIMenuWidth(),GetRageUIMenuHeight()))

RageUI.CreateWhile(1.0, RMenu:Get("AQUA:Shops", "main"), nil, function()
    RageUI.IsVisible(RMenu:Get("AQUA:Shops", "main"), true, false, true, function()
        for k, v in pairs(cfg.items) do
            if v.type == 'store' then
                ...
                end, RMenu:Get("AQUA:Shops", "sub", true))
            end
        end
    ...
    RageUI.IsVisible(RMenu:Get("AQUA:Shops", "sub"), true, false, true, function()
        ...
        RageUI.ButtonWithStyle("~g~Confirm Purchase",nil,  {RightLabel = "£"..getMoneyStringFormatted(currentItemPrice * amount)}, true, function(Hovered, Active, Selected)
            if Selected then
                TriggerServerEvent("AQUA:BuyShopItem", currentItemID, amount)
            end
        end, RMenu:Get("AQUA:Shops", "main", true))
        ...
        for k, v in pairs(cfg.shops) do
            ...
            if isInArea(v, 1.0) and inMenu == false then
                ...
                if IsControlJustPressed(0, 51) then 
                    inMenu = true
                    currentShop = k
                    RageUI.Visible(RMenu:Get("AQUA:Shops", "main"), true)
                end
            end
