...
RegisterNetEvent('AQUA:load')
AddEventHandler('AQUA:load', function(list)
	doors = list
end)

RegisterNetEvent('AQUA:statusSend')
AddEventHandler('AQUA:statusSend', function(i, status)
	...
	while true do
		...
		if not IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
			if IsControlJustPressed(table.unpack(LockHotkey)) then
				...
				if id ~= 0 then
					TriggerServerEvent("AQUA:open", id)
				end
			end
		end
	end
