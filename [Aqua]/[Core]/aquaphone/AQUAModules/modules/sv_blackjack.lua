--DiamondBlackjack created by Robbster. AQUA Edit by DocWeird.


local Tunnel = module('AQUA', 'lib/Tunnel')
local Proxy = module('AQUA', 'lib/Proxy')
AQUA = Proxy.getInterface("AQUA")
AQUAclient = Tunnel.getInterface("AQUA", "Blackjack")
...
function tryTakeChips(source,amount)
    ...
    if AQUA.tryPayment({user_id,amount}) then 
        return true
    end
end
...
AddEventHandler("Blackjack:requestSitAtBlackjackTable", function(chairId)
    ...
    if AQUA.hasGroup({userid, 'HighRoller'}) then
        if chairId > 7 then
            if source ~= nil then
                for k,v in pairs(blackjackTables) do 
                    if v == source then 
                ...
                TriggerClientEvent("blackjack:notify",source,"~d~can't sit you down.")
                --print("[casino catastrophe] id is nil what?")
            end
        else
            TriggerClientEvent("blackjack:notify",source,"~d~You Do Not Have The Correct License")
        end
    else
        if chairId >= 8 then
            TriggerClientEvent("blackjack:notify",source,"~d~You Do Not Have The Correct License")
...
AddEventHandler("Blackjack:requestSitAtBlackjackTableNorm", function(chairId)
    ...
    if AQUA.hasPermission({userid, "player.phone"}) then
                    ...
                    if tonumber(betAmount) then
                        ...
                        if betAmount > 0 then
                            if tryTakeChips(source,betAmount) then
                                ...
                                TriggerClientEvent("blackjack:notify",source,"~d~Not enough chips!")
                            end
                        end
                    end
                end
            else
                TriggerClientEvent("blackjack:notify",source,"~d~Error betting!")
            end
        else
            TriggerClientEvent("blackjack:notify",source,"~d~Minimum Amount Is 100,000")
        end
    else
        if chairId <= 7 then
            if betAmount > 100001 then
                TriggerClientEvent("blackjack:notify",source,"~d~Maximum Amount Is 100,000")
            elseif betAmount > maxHighRollersBet then
                    TriggerClientEvent("blackjack:notify",source,"~d~Maximum Amount Is £250,000,000")
            else
                                        ...
                                        while nextCardCount >= 1 do
                                            ...
                                            if blackjackGameData[gameId][source][2][nextCardCount] == true then --if hit 
                                                ...
                                                if currentHand > 21 then
                                                    ...
                                                    TriggerClientEvent("blackjack:notify",source,"~d~-"..tostring(lostAmount).." chips")
                                                    TriggerClientEvent("AQUA:PlaySound", source, "casino_lose")
                                                    local command = {
                                                        {
                                                            ["color"] = "15806495",
                                                            ["title"] = " Casino Logs",
                                                            ...
                                                            ["fields"] = {
                                                                ...
                                                                {
                                                                    ["name"] = "Player Bank",
                                                                    ["value"] = "£"..parseInt(tonumber(AQUA.getBankMoney({AQUA.getUserId({source})}))),
                                                                    ["inline"] = true
                                                                },
                                                                {
                                                                    ["name"] = "Player Cash",
                                                                    ["value"] = "£"..parseInt(tonumber(AQUA.getMoney({AQUA.getUserId({source})}))),
                                                                    ["inline"] = true
                                                                }
                                                            }
                                                        }
                                                    }
                                                    local webhook = "https://discord.com/api/webhooks/1059082082908909588/poHomQzEZJZ_zDiFPndOsNDZbdqecDVAYOm8q7H4wqa2bvb0aaAaTUczCuNiTtDX5CUT"
                                                ...
                                                if dealerHand > 21 then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent("AQUA:PlaySound", source, "casino_win")
                                                            local command = {
                                                                {
                                                                    ["color"] = "3135805",
                                                                    ["title"] = " Casino Logs",
                                                ...
                                                elseif currentHand == dealerHand then
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent("blackjack:notify",source,"~d~+0 chips")
                                                        end
                                                    end
                                                else
                                                    ...
                                                    if playerPing ~= nil then
                                                        if playerPing > 0 then
                                                            ...
                                                            TriggerClientEvent("blackjack:notify",source,"~d~-"..tostring(potentialPushAmount).." chips")
                                                            TriggerClientEvent("AQUA:PlaySound", source, "casino_lose")
                                                            local command = {
                                                                {
                                                                    ["color"] = "15806495",
