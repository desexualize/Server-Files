...
AQUAclient = Tunnel.getInterface("AQUA", "GangMenu")
...
AddEventHandler("WithdrawFunds", function(amount)
    ...
    if #(coords - bank) < 15 then
        AQUA.prompt({source, 'Enter amount to whithdraw' , "", function(source, amount)
                ...
                for k,v in pairs(funds) do 
                    AvailableGangFunds = v.gangfunds
                        if tonumber(AvailableGangFunds) < tonumber(amount) then
                            AQUAclient.notify(source,{"~d~Not enough money!"})
                        else
                            ...
                            AQUA.giveMoney({user_id, tonumber(amount)})
                            AQUAclient.notify(source,{"~g~You have withdrew £" ..Comma(tonumber(amount))})
                        end
                end
            end)
        else
            AQUAclient.notify(source,{'~d~Please enter a valid number!'})
        end
        end})
    end
end)
...
AddEventHandler("DepositFunds", function()
    ...
    AQUA.prompt({source, 'Enter amount to deposit' , "", function(source, amount)
        amount = parseInt(amount)
        if tonumber(amount) >= 0 then
            local AvailableMoney = AQUA.getMoney({user_id})
            exports['ghmattimysql']:execute("SELECT * FROM ganginfo WHERE userid = @uid", {uid = user_id}, function(result)
                fundsavailable = result
                for k,v in pairs(fundsavailable) do 
                    AvailableGangFunds = v.gangfunds
                    if AvailableMoney >= tonumber(amount) then
                        if AQUA.tryFullPayment({user_id, tonumber(amount)}) then
                            local moneyleft = AvailableGangFunds + amount
                            exports.ghmattimysql:execute("UPDATE ganginfo SET gangfunds = @money WHERE userid = @userid", {money = moneyleft, userid = user_id})
                            AQUAclient.notify(source,{"~g~Deposited: £" ..Comma(tonumber(amount))})
                        else 
                            AQUAclient.notify(source,{"~d~You do not have enough money to deposit"})
                        end
                    else 
                        AQUAclient.notify(source,{"~d~You do not have enough money to deposit"})
                    end
        ...
        exports['ghmattimysql']:execute("SELECT * FROM gangmembers WHERE userid = @uid", {uid = UserID}, function(result)
            for k,v in pairs(result) do 
                if v.name == MemberName and v.rank == OldRank then 
                    ...
                    AQUAclient.notify(source,{"Updated " ..MemberName.."'s rank to: " ..NewRank})
                end
            end
        end)
    else 
        AQUAclient.notify(source,{"Error: One field is nil!"})
    end
end)
        ...
        exports['ghmattimysql']:execute("SELECT * FROM gangmembers WHERE userid = @uid", {uid = UserID}, function(result)
            for k,v in pairs(result) do 
                if user_id == UserID then 
                    AQUAclient.notify(source,{"Cannot Kick yourself!"})
                else
                    if v.name == Name and v.rank == Rank then 
                        ...
                        AQUAclient.notify(source,{"Kicked " ..Name})
                    end
                end
            end
        end)
        ...
        exports['ghmattimysql']:execute("SELECT * FROM gangmembers WHERE userid = @uid", {uid = MemberUserID}, function(result)
            for k,v in pairs(result) do 
                if v.userid == MemberUserID and MemberRank == v.rank then
                    ...
                    AQUAclient.notify(source,{"Left your gang"})
                end
            end
        end)
    end
                ...
                exports['ghmattimysql']:execute("SELECT * FROM gangmembers WHERE userid = @uid", {uid = MemberUserID}, function(result)
                    for k,v in pairs(result) do 
                        if v.name == MemberName and v.userid == MemberUserID then 
                            exports.ghmattimysql:execute("DELETE FROM gangmembers WHERE userid = @userid", {userid = v.userid})
                            AQUAclient.notify(source,{"Disbanded Gang"})
                        end
                    end
                end)
            end
            ...
            if gang == "[]" then 
                exports['ghmattimysql']:execute("SELECT * FROM ganginfo WHERE gangname = @gangname", {gangname = name}, function(result)
                    ...
                    if gang == "[]" then 
                        ...
                        AQUAclient.notify(source,{"Created Gang!"})
                        TriggerClientEvent("closemenu", source)
                    else 
                        AQUAclient.notify(source,{"Error: Gang Name/Leader is already taken"})
                    end
                end)
            else
                AQUAclient.notify(source,{"You are already in a gang!"})
            end
        end)
    else
        AQUAclient.notify(source,{'~d~You need a gang license to create a gang.'})
    end
end)
        ...
        exports['ghmattimysql']:execute("SELECT * FROM gangmembers WHERE userid = @uid", {uid = userid}, function(result)
            ...
            if gang == "[]" then 
                exports['ghmattimysql']:execute("SELECT * FROM ganginfo WHERE gangname = @gangname", {gangname = GangNameToJoin}, function(result)
                    ...
                    if ganginfo == "[]" then 
                        AQUAclient.notify(source,{"That gang does not exist, invite has expired!"})
                    else
                        ...
                        AQUAclient.notify(source,{"You have joined the gang: " ..GangNameToJoin.. " !"})
                    end
                end)
            else
                AQUAclient.notify(source,{"You are already in a gang!"})
...
AddEventHandler("MemberInvites", function(PermToInv, GangName, GangLeader)
    ...
    MyPermID = AQUA.getUserId({source})
    if tonumber(PermToInv) == tonumber(MyPermID) then 
        AQUAclient.notify(src,{"Error: You cannot invite yourself!"})
        return
    end
