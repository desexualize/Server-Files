...
function AQUA.openMenu(source,menudef)
  local menudata = {}
  menudata.choices = {}
  ...
  -- openmenu
  AQUAclient.openMenuData(source,{menudata})
end

-- force close player menu
function AQUA.closeMenu(source)
  AQUAclient.closeMenu(source,{})
end
...
function AQUA.prompt(source,title,default_text,cb_result)
  prompts[source] = cb_result

  AQUAclient.prompt(source,{title,default_text})
end
...
function AQUA.request(source,text,time,cb_ok)
  local id = request_ids:gen()
  local request = {source = source, cb_ok = cb_ok, done = false}
  requests[id] = request

  AQUAclient.request(source,{id,text,time}) -- send request to client
...
function AQUA.registerMenuBuilder(name, builder)
...
function AQUA.buildMenu(name, data, cbr)
...
function AQUA.openMainMenu(source)
  AQUA.buildMenu("main", {player = source}, function(menudata)
    menudata.name = "Main menu"
    menudata.css = {top="75px",header_color="rgba(0,125,255,0.75)"}
    AQUA.openMenu(source,menudata) -- open the generated menu
  end)
end

-- SERVER TUNNEL API

function tAQUA.closeMenu(id)
...
function tAQUA.validMenuChoice(id,choice,mod)
  local menu = client_menus[id]
  if menu and menu.source == source then
...
function tAQUA.promptResult(text)
  if text == nil then
    text = ""
  end
...
function tAQUA.requestResult(id,ok)
...
function tAQUA.openMainMenu()
  AQUA.openMainMenu(source)
end
...
function AQUA.addStaticMenuChoices(name, choices)
...
-- build static menus
local static_menus = {}
SetTimeout(10000,function() -- wait for AQUA.addStaticMenuChoices calls
    ...
    for k,v in pairs(cfg.static_menus) do
      ...
      if menu and smenu then
        local function smenu_enter()
          local user_id = AQUA.getUserId(source)
          if user_id ~= nil and AQUA.hasPermissions(user_id,smenu.permissions or {}) then
        ...
        AQUAclient.addBlip(source,{x,y,z,smenu.blipid,smenu.blipcolor,smenu.title})
        AQUAclient.addMarker(source,{x,y,z-1,0.7,0.7,0.5,255,226,0,125,150})

        AQUA.setArea(source,"AQUA:static_menu:"..k,x,y,z,1,1.5,smenu_enter,smenu_leave)
      end
    end
  end
end
...
AddEventHandler("AQUA:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    ...
    AQUAclient.setDiv(source,{"additional_css",".div_additional_css{ display: none; }\n\n"..cfg.css,""})
