...
function AQUA.getUserGroups(user_id)
...
function AQUA.addUserGroup(user_id, group)
    print("[ID]: " .. user_id.. " Got group: ".. group)
    if not AQUA.hasGroup(user_id, group) then
        local user_groups = AQUA.getUserGroups(user_id)
        ...
        if ngroup then
            if ngroup._config and ngroup._config.gtype ~= nil then
                ...
                for k, v in pairs(_user_groups) do -- remove all groups with the same gtype
                    ...
                    if kgroup and kgroup._config and ngroup._config and kgroup._config.gtype == ngroup._config.gtype then
                        AQUA.removeUserGroup(user_id, k)
                    end
                end
            end
            ...
            TriggerEvent("AQUA:playerJoinGroup", user_id, group, gtype)
        end
    end
end
...
function AQUA.getUserGroupByType(user_id, gtype)
    local user_groups = AQUA.getUserGroups(user_id)
    for k, v in pairs(user_groups) do
        local kgroup = groups[k]
        if kgroup then
...
function AQUA.getUsersByGroup(group)
    local users = {}

    for k, v in pairs(AQUA.rusers) do
        if AQUA.hasGroup(tonumber(k), group) then
...
function AQUA.getUsersByPermission(perm)
    local users = {}

    for k, v in pairs(AQUA.rusers) do
        if AQUA.hasPermission(tonumber(k), perm) then
            table.insert(users, tonumber(k))
        end
    end
...
function AQUA.removeUserGroup(user_id, group)
    local user_groups = AQUA.getUserGroups(user_id)
    ...
    TriggerEvent("AQUA:playerLeaveGroup", user_id, group, gtype)

    user_groups[group] = nil -- remove reference
end

-- check if the user has a specific group
function AQUA.hasGroup(user_id, group)
    local user_groups = AQUA.getUserGroups(user_id)
...
-- check if the user has a specific permission
function AQUA.hasPermission(user_id, perm)
    local user_groups = AQUA.getUserGroups(user_id)
    ...
    if fchar == "@" then -- special aptitude permission
        ...
        if #parts == 3 then -- decompose group.aptitude.operator
            ...
            local alvl = math.floor(AQUA.expToLevel(AQUA.getExp(user_id, group, aptitude)))
    ...
    elseif fchar == "#" then -- special item permission
        ...
        if #parts == 2 then -- decompose item.operator
            ...
            local amount = AQUA.getInventoryItemAmount(user_id, item)
...
function AQUA.hasPermissions(user_id, perms)
...
local function ch_select(player, choice)
    ...
    if user_id ~= nil then
        AQUA.addUserGroup(user_id, choice)
        ...
        for k, v in pairs(selectors) do
            ...
            if gcfg and menu then
                ...
                local function selector_enter()
                    local user_id = AQUA.getUserId(source)
                    if user_id ~= nil and AQUA.hasPermissions(user_id, gcfg.permissions or {}) then
                ...
                AQUAclient.addBlip(source, {x, y, z, gcfg.blipid, gcfg.blipcolor, k})
                AQUAclient.addMarker(source, {x, y, z - 1, 0.7, 0.7, 0.5, 255, 154, 24, 125, 150})

                AQUA.setArea(source, "AQUA:gselector:" .. k, x, y, z, 1, 1.5, selector_enter, selector_leave)
            end
        end
    end
end
    ...
    if first_spawn then
        ...
        if user ~= nil then
            for k, v in pairs(user) do
                AQUA.addUserGroup(user_id, v)
            end
        end

        -- add default group user
        AQUA.addUserGroup(user_id, "user")
    end
