...
local cfg = module("AQUAVehicles", "cfg/cfg_inventory")
    ...
    --   TriggerClientEvent('ECP:ToggleNUIFocus', player, false)
      AQUA.prompt(player,lang.inventory.trash.prompt({AQUA.getInventoryItemAmount(user_id,idname)}),"",function(player,amount)
        ...
        --   TriggerClientEvent('ECP:ToggleNUIFocus', player, true)
          TriggerEvent('AQUA:RefreshInventory', AQUA.getUserSource(user_id))
          ...
          AQUA.prompt(player,lang.inventory.give.prompt({AQUA.getInventoryItemAmount(user_id,idname)}),"",function(player,amount)
            ...
            if new_weight <= AQUA.getInventoryMaxWeight(nuser_id) then
              if AQUA.tryGetInventoryItem(user_id,idname,amount,true) then
                ...
                TriggerEvent('AQUA:RefreshInventory', nplayer)
...
function AQUA.giveInventoryItem(user_id,idname,amount,notify)
  ...
  if data and amount > 0 then
    ...
    TriggerClientEvent('AQUA:FetchInventoryCL', AQUA.getUserSource(user_id))
...
function AQUA.RunTrashTask(source, itemName)
    local choices = AQUA.getItemChoices(itemName)
    ...
    TriggerEvent('AQUA:RefreshInventory', source)
end


function AQUA.RunGiveTask(source, itemName)
    local choices = AQUA.getItemChoices(itemName)
...
function AQUA.RunInventoryTask(source, itemName)
    local choices = AQUA.getItemChoices(itemName)
    if choices['Use'] then 
...
function AQUA.getUserItemAmount(user_id,idname)
  local data = AQUA.getUserDataTable(user_id)
...
function AQUA.getUserHasItem(user_id,idname)
  local data = AQUA.getUserDataTable(user_id)
