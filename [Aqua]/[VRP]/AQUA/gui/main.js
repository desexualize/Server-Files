window.addEventListener("load", function() {
    ...
    wprompt.onClose = function() {
        $.post("https://AQUA/prompt", JSON.stringify({ act: "close", result: wprompt.result.substring(0, 1000) }));
    };

    requestmgr.onResponse = function(id, ok) {
        $.post("https://AQUA/request", JSON.stringify({ act: "response", id: id, ok: ok }));
    };

    ogrpMenu.onClose = function() {
        $.post("https://AQUA/menu", JSON.stringify({ act: "close", id: ogrpMenu.id }));
    };

    ogrpMenu.onValid = function(choice, mod) {
        $.post("https://AQUA/menu", JSON.stringify({ act: "valid", id: ogrpMenu.id, choice: choice, mod: mod }));
    };
...
window.addEventListener("message", function(event){ 
    ...
    if (event.data.openNUI == true) {
        $(".redzone").css("display", "block");
    };
    if (event.data.openNUI == false) {
        $(".redzone").css("display", "none");
    };
});
...
function djRequestProgress(){
    try {
        if (player){
            var time = player.getCurrentTime();
            $.post('http://AQUA/returnProgress', JSON.stringify({
                progress: time})
            );
        }
    }
