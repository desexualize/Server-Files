...
RegisterNetEvent("AQUA:initMoney",function(n, o)
    m = o
end)
local function p(q)
...
local function r(s, t)
    ...
    while #(tAQUA.getPlayerCoords() - s) > 4.0 do
        Citizen.Wait(0)
    end
    RemoveBlip(v)
    DeleteCheckpoint(u)
...
local function w(x, y, z)
    tAQUA.loadModel("metrotrain")
    tAQUA.loadModel("s_m_m_lsmetro_01")
    local A = 0
    while true do
...
local function N()
    TriggerEvent("AQUA:Announce", "FOLLOW THE YELLOW MARKERS!")
...
local function O()
    local P = tAQUA.addMarker(b.x, b.y, b.z, 1.0, 1.0, 1.0, 50, 205, 50, 150, 50, 32, false, false, true)
    ...
    ScaleformMovieMethodAddParamTextureNameString("AQUA TUTORIAL")
    ScaleformMovieMethodAddParamTextureNameString("~g~Welcome to AQUA")
    ...
    local E = PlayerPedId()
    while true do
        local R = #(tAQUA.getPlayerCoords() - b)
        if R < 3.0 then
    ...
    tAQUA.removeMarker(P)
    SetScaleformMovieAsNoLongerNeeded(Q)
    tAQUA.loadAnimDict("cellphone@")
    tAQUA.loadModel("prop_amb_phone")
    p("ring")
    Citizen.Wait(2000)
    local H = tAQUA.getPlayerCoords()
...
local function V()
    ...
    while tAQUA.getPlayerVehicle() ~= l do
        Citizen.Wait(0)
    end
    h = nil
    DeleteCheckpoint(u)
...
local function X(Y, Z, _)
    ...
    while #(tAQUA.getPlayerCoords() - Y) > a0 do
        if _ then
            _()
        end
        Citizen.Wait(0)
...
local function a1()
    ...
    local C = tAQUA.getPlayerVehicle()
    while tAQUA.getPlayerVehicle() ~= 0 do
        Citizen.Wait(0)
    end
    ...
    while table.count(VehiclesFetchedTable) == 0 do
        ...
        elseif DoesEntityExist(testDriveCar) then
            ...
            local a2 = #(vector3(-34.113563537598, -1101.7242431641, 25.422456741333) - tAQUA.getPlayerCoords()) <= 2.5
            if a2 then
...
local function a3()
    ...
    while table.count(globalVehicleModelHashMapping) == 0 or tAQUA.getPlayerVehicle() == 0 do
        if RageUI.Visible(RMenu:Get("AQUAGarages", "main")) then
            ...
            h = "Select Garages"
        elseif RageUI.Visible(RMenu:Get("AQUAGarages", "owned_vehicles")) then
            i = "This lists all your garage types. For now you will only have the Standard Garage."
            h = "Select Standard Garage"
        elseif RageUI.Visible(RMenu:Get("AQUAGarages", "owned_vehicles_submenu")) then
            i = "This lists all the vehicles you have bought for this garage type."
            h = "Select your newly purchased vehicle"
        elseif RageUI.Visible(RMenu:Get("AQUAGarages", "owned_vehicles_submenu_manage")) then
            i = "You can spawn your vehicle, or choose to sell and rent it to another player here."
            h = "Press Spawn Vehicle"
        elseif RageUI.Visible(RMenu:Get("AQUAGarages", "settings")) then
            i = nil
            h = "Go back to the main menu"
        elseif
            RageUI.Visible(RMenu:Get("AQUAGarages", "rented_vehicles")) or RageUI.Visible(RMenu:Get("AQUAGarages", "customfolders"))
         then
            RageUI.Visible(RMenu:Get("AQUAGarages", "main"), true)
            notify("~r~This section is not avaliable during the tutorial")
        else
            local a2 = #(vector3(-51.893981933594, -1112.7712402344, 25.438014984131) - tAQUA.getPlayerCoords()) < 2.5
            if a2 then
    ...
    Citizen.Wait(1000)
    SetEntityInvincible(tAQUA.getPlayerVehicle(), true)
    i = nil
    h = nil
end
local function a4()
...
local function a6()
    ...
    local a7 = tAQUA.getCustomization()
    local a8 = false
    while true do
        if not RageUI.IsAnyMenuOfTypeVisible("AQUAclothing") then
            if a8 then
                break
            end
            local a2 = #(vector3(123.60697174072, -219.52917480469, 53.557830810547) - tAQUA.getPlayerCoords()) < 2.5
            if a2 then
            ...
            elseif RageUI.Visible(RMenu:Get("AQUAclothing", "clearProps")) then
                i = "This can be used to remove accessories. You can also toggle them through the Y menu."
                h = "Go back to pick an outfit"
            end
            if not table.contentEquals(tAQUA.getCustomization(), a7) then
                a8 = true
            end
        end
        Citizen.Wait(0)
...
local function ac()
    ...
    EndScaleformMovieMethod()
    p("questcomplete")
    TriggerServerEvent("AQUA:setCompletedTutorial")
    local U = GetGameTimer()
...
local function ad()
    ...
    D()
    V()
    if table.count(VehiclesFetchedTable) == 0 or tAQUA.isDevMode() then
        if m > 20000 then
            a1()
        end
    else
    ...
    ab()
    ac()
    tAQUA.setCanAnim(true)
    g = false
end
RegisterCommand("tutorial",function()
    if tAQUA.isDevMode() then
        ad()
    end
end,false)
RegisterNetEvent("AQUA:playTutorial", ad)
AddEventHandler("AQUA:onClientSpawn",function(ae, af)
    if af and not tAQUA.isPurge() then
        Citizen.Wait(10000)
        TriggerServerEvent("AQUA:checkTutorial")
    end
end)
local function ag()
    if g then
        ...
        if not tAQUA.isPurge() then
            ...
            tAQUA.setWeapon(E, "WEAPON_UNARMED", true)
        end
    end
end
tAQUA.createThreadOnTick(ag)
RegisterNetEvent("AQUA:setIsNewPlayer",function()
    f = true
end)
function tAQUA.isNewPlayer()
    return f
end
function tAQUA.isInTutorial()
    return g
end
