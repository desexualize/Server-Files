...
function tAQUA.toggleHandcuff()
  ...
  if handcuffed then
    ...
    tAQUA.playAnim(true,{{"mp_arresting","idle",1}},true)
  else
    DeleteEntity(cuffs)
    tAQUA.stopAnim(true)
...
function tAQUA.setHandcuffed(flag)
  if handcuffed ~= flag then
    tAQUA.toggleHandcuff()
  end
end

function tAQUA.isHandcuffed()
  return handcuffed
end

-- (experimental, based on experimental getNearestVehicle)
function tAQUA.putInNearestVehicleAsPassenger(radius)
...
function tAQUA.putInNetVehicleAsPassenger(net_veh)
...
function tAQUA.putInVehiclePositionAsPassenger(x,y,z)
  local veh = tAQUA.getVehicleAtPosition(x,y,z)
  if IsEntityAVehicle(veh) then
    for i=1,GetVehicleMaxNumberOfPassengers(veh) do
      if IsVehicleSeatFree(veh,i) then
        SetPedIntoVehicle(GetPlayerPed(-1),veh,i)
...
function tAQUA.jail(x,y,z,radius)
  tAQUA.teleport(x,y,z) -- teleport to center
  jail = {x+0.0001,y+0.0001,z+0.0001,radius+0.0001}
end

-- unjail the player
function tAQUA.unjail()
  jail = nil
end

function tAQUA.isJailed()
  return jail ~= nil
end
