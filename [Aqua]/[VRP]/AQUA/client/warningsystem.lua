...
RegisterNetEvent("AQUA:showWarningsOfUser")
AddEventHandler("AQUA:showWarningsOfUser",function(AQUAwarningstables)
	showWarningSystem = true
	AQUAWarnings = AQUAwarningstables
end)

RegisterNetEvent("AQUA:recievedRefreshedWarningData")
AddEventHandler("AQUA:recievedRefreshedWarningData",function(AQUAwarningstables)
	AQUAWarnings = AQUAwarningstables
end)
...
RegisterCommand('warnings', function()
    ...
    if showWarningSystem then
        TriggerServerEvent("AQUA:refreshWarningSystem")
    end
end)
...
Citizen.CreateThread(function()
	while true do
		if showWarningSystem then
			...
			DrawAdvancedText(0.59, 0.198, 0.005, 0.0028, 0.619, ' Punishments', 255, 255, 255, 255, 7, 0)
			...
			for warningID,warningTable in pairs(AQUAWarnings) do
