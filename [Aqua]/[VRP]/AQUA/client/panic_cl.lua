...
tAQUA.hasPoliceJob = function(bool)
    police = bool
end

tAQUA.PanicCL = function(player, s1, s2, name)
    if police then
...
tAQUA.BlipCL = function(tx, ty, tz)
    if police then
    ...
    while true do
        if showPanicBlip and police and not panic then
            ...
            if tostring(GetStreetNameFromHashKey(streetA)) and tostring(GetStreetNameFromHashKey(streetB)) then
                ...
                AQUAserver.PanicSV({player,  GetStreetNameFromHashKey(streetA), GetStreetNameFromHashKey(streetB)})
                AQUAserver.BlipSV({plyPos.x, plyPos.y, plyPos.z})
            else
                AQUAserver.PanicSV({player,  "No street found", ""})
                AQUAserver.BlipSV({plyPos.x, plyPos.y, plyPos.z})
            end
        end
