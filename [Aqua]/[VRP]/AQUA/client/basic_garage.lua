...
function tAQUA.spawnGarageVehicle(vtype, name, pos, vehiclename, vehicleplate) -- vtype is the vehicle type (one vehicle per type allowed at the same time)
    ...
    if vehicle == nil then
        ...
        if HasModelLoaded(mhash) then
            ...
            TriggerServerEvent("AQUA:applyModifications", name, nveh)
            TriggerServerEvent("AQUA:PayVehicleTax")
        ...
        tAQUA.notify("~d~Vehicle " ..vehiclename.. " is already out.")
    end
end

function tAQUA.despawnGarageVehicle(vtype, max_range)
    local Vehicle = tAQUA.getNearestVehicle(3)
...
function tAQUA.getNearestVehicl2()
    local ped = GetPlayerPed(-1)
    if IsPedSittingInAnyVehicle(ped) then
...
function tAQUA.fixeNearestVehicle(radius)
    local veh = tAQUA.getNearestVehicle(radius)
    if IsEntityAVehicle(veh) then
        SetVehicleFixed(veh)
    end
end

function tAQUA.replaceNearestVehicle(radius)
    local veh = tAQUA.getNearestVehicle(radius)
    if IsEntityAVehicle(veh) then
...
function tAQUA.getVehicleAtPosition(x, y, z)
    x = x + 0.0001
    y = y + 0.0001
    z = z + 0.0001
...
function tAQUA.getNearestOwnedVehicle(radius)
...
-- return ok,x,y,z
function tAQUA.getAnyOwnedVehiclePosition()
    for k, v in pairs(vehicles) do
...
-- return x,y,z
function tAQUA.getOwnedVehiclePosition(vtype)
...
function tAQUA.getOwnedVehicleId(vtype)
    local vehicle = vehicles[vtype]
    if vehicle then
...
-- vehicle commands
function tAQUA.vc_openDoor(hash, door_index)
...
function tAQUA.vc_closeDoor(hash, door_index)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tAQUA.vc_detachTrailer(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tAQUA.vc_detachTowTruck(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tAQUA.vc_detachCargobob(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tAQUA.vc_toggleEngine(hash)
    local vehicle = vehicles[hash]
    if vehicle then
...
function tAQUA.vc_toggleLock(hash)
    local vehicle = vehicles[hash]
    if vehicle then
        local veh = vehicle[3]
        ...
        if locked then -- unlock
            ...
            tAQUA.notify("~d~Vehicle unlocked.")
        else -- lock
            SetVehicleDoorsLocked(veh, 2)
            SetVehicleDoorsLockedForAllPlayers(veh, true)
            tAQUA.notify("~g~Vehicle locked.")
        end
    end
end
