...
local function DriveOutOfGarage(pos)
	Citizen.CreateThread(function()
		...
		local ok, veh_type, model = tAQUA.getNearestOwnedVehicle(1)
		myveh.model = model
						...
						if LSC_Config.oldenter then			
							...
							if #(vec3(outside.x,outside.y,outside.z) - GetEntityCoords(ped)) <= f(5) then
								...
								if not tableContains(LSC_Config.ModelBlacklist,GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
									...
									if pos.locked then
										...
										if not LSC_Config.lock then
											if IsControlJustPressed(1,201) then
												...
												drawTxt("Press ~d~ENTER~w~ to enter ~d~Los Santos Customs ",4,1,0.5,0.8,1.0,255,255,255,255)
											end
										else
											drawTxt("~d~Locked, please wait",4,1,0.5,0.8,1.0,255,255,255,255)
										end
									else
										if IsControlJustPressed(1,201) then
											inside = true
									...
									drawTxt("~d~This vehicle can't be upgraded",4,1,0.5,0.8,1.0,255,255,255,255)
								end
							end
						else
...
function LSCMenu:onSelectedIndexChanged(name, button)
	...
	elseif m == "neon color" then
		if myveh.neon then
			...
			LSCMenu:showNotification("~d~You don't have neon lights installed.")
		end
	elseif m == "xenon color" then
		if myveh.mods[22].mod == 1 then
			SetVehicleHeadlightsColour(veh, button.xenon)
		else
			LSCMenu:showNotification("~d~You don't have xenon headlights installed.")
		end
...
function CanPurchase(price, canpurchase)
	if canpurchase then
		...
		LSCMenu:showNotification("~d~You cannot afford this purchase.")
		return false
	end
end
